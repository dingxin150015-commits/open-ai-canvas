...
6. 本项目开发、管理后台检查和浏览器联调统一使用 Microsoft Edge，并复用用户已登录的 Edge 会话；不要使用 Chrome，也不要在 Edge 不可用时自行回退到 Chrome 或其他浏览器。若 Edge 登录失效或连接不可用，停止浏览器步骤并请用户处理或授权下一步。
...
本项目的持久工程记忆入口是 `.codex/project-memory/README.md`。开始任何非平凡任务前，先阅读该入口，并按任务需要继续读取现状、时间线、经验教训和待办计划；不要把 `.claude/` 中标题带“完成”“确认”的历史报告直接视为当前事实。

完成以下事项后必须同步更新项目记忆：

- 功能、架构、协议、数据结构、部署方式或安全边界发生变化；
- 问题被确认、推翻、修复或完成运行验证；
- 出现数据事故、静默覆盖、回滚、迁移或重要工具链故障；
- GitHub、上游、分支、容器、数据库、模型供应商或授权状态发生变化；
- 待办、执行顺序、NO-GO 门禁或用户批准范围发生变化。

记忆内容必须区分“当前已确认”“历史记录”“推测”“待验证”，并附提交、文件、测试、日志或文档证据。不要记录 Token、Cookie、API Key、密码、完整敏感 URL 或用户数据正文。
