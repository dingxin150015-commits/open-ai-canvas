...
export function CanvasAssetTray({ assetImages, canvasImages, showLibrary = true, activeNodeId, onInsertAssetImage, onFocusCanvasImage, compact = false }: CanvasAssetTrayProps) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const reducedMotion = useReducedMotion();
    const { bringToFront, zIndex } = useCanvasOverlayLayer("asset-tray", "var(--z-panel)");
    const rootRef = useRef<HTMLDivElement>(null);
    const [open, setOpen] = useState(false);
    const panelLeft = useCanvasPanelOffset(open, rootRef, 312);
    ...
    const dockItems: FloatingDockEntry[] = [
        {
            ...
            icon: <ImageIcon />,
            displayLabel: String(Math.min(99, (showLibrary ? assetImages.length : 0) + canvasImages.length)),
            active: open,
            onClick: () => {
                bringToFront();
                setOpen((value) => !value);
    ...
    return (
        ...
        >
            ...
            <FloatingDock items={dockItems} size={compact ? "compact" : "default"} maxVisible={1} showLabels className="canvas-floating-dock w-fit" style={canvasDockStyle(theme)} ariaLabel="素材空间" />
        </div>
    );
}
