...
import { canvasAppearanceForTheme, customCanvasAppearanceFromTheme, enterCustomCanvasAppearance, normalizeHexColor, type CanvasAppearance, type CanvasCustomAppearance } from "@/lib/canvas/canvas-appearance";
...
export function CanvasAppearanceControls({
    ...
    const updateCustom = (patch: Partial<CanvasCustomAppearance>) => {
        const current = draft.mode === "custom" && draft.custom ? draft : enterCustomCanvasAppearance(draft, colorTheme);
    ...
    return (
        <>
            ...
            <div className="mt-1 grid grid-cols-3 gap-1 rounded-[var(--dock-item-radius-labeled)] border p-1" style={{ background: theme.spatial.surface, borderColor: theme.toolbar.border }}>
                <ThemeButton active={draft.mode === "light"} label="浅色" theme={theme} onClick={() => selectFixedTheme("light")}>
                    <Sun className="size-3.5" />
                </ThemeButton>
                <ThemeButton active={draft.mode === "dark"} label="深色" theme={theme} onClick={() => selectFixedTheme("dark")}>
                    <Moon className="size-3.5" />
                </ThemeButton>
                <ThemeButton active={draft.mode === "custom"} label="自定义" theme={theme} onClick={selectCustomTheme}>
            ...
            {draft.mode === "custom" && draft.custom ? (
                <div className="mt-2.5 space-y-2.5 rounded-[var(--dock-item-radius-labeled)] border p-2.5" style={{ background: theme.spatial.surface, borderColor: theme.toolbar.border }}>
                    ...
                    <div className="grid grid-cols-4 gap-1.5">
                        {presets.map((color) => (
                            <button
                                key={color}
                                type="button"
                                className="h-7 rounded-md border transition-transform hover:scale-[1.04] focus-visible:outline focus-visible:outline-2 motion-reduce:transition-none motion-reduce:hover:scale-100"
                                style={{ background: color, borderColor: draft.custom?.backgroundColor === color ? theme.node.activeStroke : theme.toolbar.border }}
                                aria-label={`使用背景颜色 ${color}`}
                                title={color}
                                onClick={() => updateCustom({ backgroundColor: color, backgroundBrightness: 0 })}
                            />
                        ))}
                    </div>
                    <Segmented
                    ...
                    <Button block size="small" type="text" icon={<RotateCcw className="size-3.5" />} onClick={resetCustom}>
                        从继承主题重新开始
                    </Button>
                    <Button block size="small" type="primary" icon={<Save className="size-3.5" />} onClick={saveAsDefault}>
                        保存为默认
                    </Button>
                </div>
            ) : null}
...
function ThemeButton({ active, label, theme, onClick, children }: { active: boolean; label: string; theme: CanvasTheme; onClick: () => void; children: ReactNode }) {
    return (
        <button
            type="button"
            className="inline-flex h-8 min-w-0 items-center justify-center gap-1 rounded-[var(--dock-item-radius)] px-1 text-[var(--fs-tiny)] font-semibold transition-colors"
