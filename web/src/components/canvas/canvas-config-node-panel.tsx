...
export function CanvasConfigNodePanel({ node, isRunning, inputSummary, onConfigChange, onGenerate, onComposerToggle, workspaceMode = "professional" }: CanvasConfigNodePanelProps) {
    ...
    const selectedWorkflowFields: WorkflowVideoFieldLike[] =
        workflowProvider === "runninghub"
            ...
            : workflowProvider === "comfyui"
              ? selectedComfyBridgeWorkflow?.fields?.length
                  ? selectedComfyBridgeWorkflow.fields
                  : workflowVideoFieldsFromJson(selectedComfyBridgeWorkflow?.workflowJson)
              : [];
    ...
    useEffect(() => {
        ...
        globalConfig.comfyBridge.workflowId,
        ...
        mode,
        node.id,
        node.metadata?.comfyBridgeWorkflowId,
            ...
            : workflowProvider === "comfyui"
              ? !workflowProviderPluginEnabled(runtimeStatuses, "comfyui")
                  ...
                  : !globalConfig.comfyBridge.enabled
                    ? "请先在设置中启用 ComfyUI Bridge"
                    : !globalConfig.comfyBridge.bridgeId
                      ? "请选择在线 Bridge"
                      : !node.metadata?.comfyBridgeWorkflowId
                        ? `请选择${capabilityLabel(workflowCapability)}工作流`
                        : !selectedComfyBridgeWorkflow
                          ? "当前画布引用的 ComfyUI 条目已不存在，请重新选择"
                          : selectedComfyBridgeWorkflow.capability !== workflowCapability
                            ? `当前条目用途为${capabilityLabel(selectedComfyBridgeWorkflow.capability)}，请切换画布模式或重新选择条目`
                            : undefined
              : undefined);
...
function buildNodeConfig(globalConfig: AiConfig, node: CanvasNodeData, mode: CanvasGenerationMode, requirements: ModelRequirements): AiConfig {
    ...
    const selectedRunningHubWorkflow = selectedRunningHubWorkflowId
        ...
        : undefined;
    const selectedComfyBridgeWorkflow = selectedComfyBridgeWorkflowId ? globalConfig.comfyBridge.workflows.find((item) => item.workflowId.trim() === selectedComfyBridgeWorkflowId) : undefined;
    ...
    const comfyBridge = {
        ...globalConfig.comfyBridge,
        enabled: mode !== "text" && workflowProvider === "comfyui" && globalConfig.comfyBridge.enabled,
        workflowId: selectedComfyBridgeWorkflowId || "",
        capability: selectedComfyBridgeWorkflow?.capability || modeCapability,
    };
    return {
        ...globalConfig,
