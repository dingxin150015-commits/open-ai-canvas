...
export function CanvasDirectorWorkbench({
    ...
    const commit = useCallback(
        (updater: (current: DirectorScene) => DirectorScene) => {
    ...
    const stagedTransaction = useMemo(
        () =>
            createDirectorTransaction<DirectorScene>({
    ...
    useEffect(
        () =>
            installDirectorTerminalListeners(stagedTransaction, {
                window,
                document,
    ...
    const addCameraKeyframe = () => {
        ...
        commit((current) => ({
            ...
            cameras: current.cameras.map((item) =>
                item.id === activeCamera.id ? { ...item, keyframes: resolveDirectorKeyframeRecord({ base: item.transform, keyframes: item.keyframes, rawTime: playhead, snappedTime: snappedPlayhead }).keyframes } : item,
            ),
        }));
    };
    ...
    const setKeyframeEasing = useCallback(
        (target: DirectorKeyframeDeleteTarget, easing: DirectorKeyframeEasing) => {
            const current = draftRef.current;
    ...
    const handleObjectTransform = useCallback(
        (id: string, from: DirectorTransform, to: DirectorTransform) => {
            commit((current) => ({
                ...current,
    ...
    const writeBoneRotation = useCallback(
        (id: string, bone: string, rotation: DirectorQuat, mode: "stage" | "commit") => {
    ...
    const applyCameraMove = () => {
        ...
        commit((current) => ({
            ...current,
            cameras: current.cameras.map((item) => (item.id === cameraId ? { ...item, keyframes: resolveDirectorCameraMoveKeyframes(item.keyframes, item.transform, cameraMoveTransform(item.transform, move), duration) } : item)),
        }));
    ...
    return (
        <div data-canvas-no-zoom className="fixed inset-0 z-[var(--z-toast)] flex min-h-0 flex-col overflow-hidden" style={{ background: theme.canvas.background, color: theme.node.text }}>
            <header className="thin-scrollbar flex h-12 shrink-0 items-center gap-2 overflow-x-auto overflow-y-hidden border-b px-2" style={{ background: theme.toolbar.panel, borderColor: theme.toolbar.border }}>
                <IconButton label="关闭导演台" onClick={closeWorkbench}>
                    <X className="size-4" />
                </IconButton>
                ...
                <IconButton label="撤销" disabled={!history.length} onClick={undo}>
                    <Undo2 className="size-4" />
                </IconButton>
                <IconButton label="重做" disabled={!future.length} onClick={redo}>
                ...
                <div className="ml-auto flex items-center gap-2">
                    <span aria-live="polite" className="text-[var(--fs-tiny)]" style={{ color: saveIndicator.tone === "danger" ? "var(--status-error)" : undefined, opacity: saveIndicator.tone === "idle" ? 0.55 : 1 }}>
                        {saveIndicator.label}
                    </span>
                    {saveIndicator.retryable ? (
                        <Button size="small" icon={<RotateCcw className="size-3.5" />} loading={retrying || saveIndicator.busy} onClick={() => void retrySave()}>
                            重试保存
                        </Button>
                    ) : null}
                </div>
                <div className="flex items-center gap-1">
                    {onboardingScope ? (
                        <IconButton label="重新开始引导" onClick={() => setOnboardingRestartSignal((value) => value + 1)}>
            ...
            <div className="grid min-h-0 flex-1 grid-cols-[220px_minmax(0,1fr)_292px] max-lg:grid-cols-[180px_minmax(0,1fr)]">
                ...
                <main className="relative min-h-0 overflow-hidden bg-neutral-900">
                    ...
                    <div className="pointer-events-none absolute left-3 top-3 text-[var(--fs-tiny)] font-medium text-white/70">
                        {activeShot.name} · {activeCamera?.name || "无摄影机"} · {activeShot.duration}s
                    </div>
