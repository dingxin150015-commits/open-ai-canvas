...
export function CanvasNodePanelOverlay({
    ...
    useLayoutEffect(() => {
        ...
        const update = (nextViewport: ViewportTransform) => {
            ...
            const position = getNodePanelPosition(node, nextViewport, viewportSize, nextWidth, panelHeight, dragOffset);
...
export function CanvasConnectionCreateMenu({
    ...
    getDisabledReason: (type: CanvasNodeType.Image | CanvasNodeType.Text | CanvasNodeType.Script | CanvasNodeType.Video | CanvasNodeType.Audio | CanvasNodeType.Drawing | CanvasNodeType.Config, provider?: "runninghub" | "comfyui") => string;
    onCreate: (type: CanvasNodeType.Image | CanvasNodeType.Text | CanvasNodeType.Script | CanvasNodeType.Video | CanvasNodeType.Audio | CanvasNodeType.Drawing | CanvasNodeType.Config, provider?: "runninghub" | "comfyui") => void;
    onClose: () => void;
}) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const reducedMotion = useReducedMotion();
    ...
    return (
        ...
        >
            ...
            <div className="mb-1.5 flex items-center justify-between gap-2 px-1 py-0.5">
                <span className="flex min-w-0 items-center gap-2">
                    ...
                    <span className="min-w-0">
                        <span className="block truncate text-[var(--fs-label)] font-semibold">创建下一步</span>
                        <span className="mt-0.5 block truncate text-[var(--fs-micro)]" style={{ color: theme.node.muted }}>
                            {pending.batchSourceNodeIds?.length ? `引用已选 ${pending.batchSourceNodeIds.length} 个节点` : "引用当前节点"}
                        </span>
                    </span>
                </span>
                <button
            ...
            <div className="grid min-w-0 grid-cols-1 gap-1">
                ...
                />
                {canCreateDrawing ? (
                    <ConnectionCreateOption motionEnabled={!reducedMotion} icon={<Pencil className="size-4" />} title="绘图" disabledReason={getDisabledReason(CanvasNodeType.Drawing)} onClick={() => onCreate(CanvasNodeType.Drawing)} />
                ) : null}
        ...
        >
            ...
            <span className="min-w-0 flex-1">
                ...
                {disabledReason || description ? (
                    <span className="mt-0.5 block whitespace-normal break-words text-[var(--fs-micro)] leading-relaxed" style={{ color: theme.node.muted }}>
                        {disabledReason || description}
                    </span>
                ) : null}
            </span>
