            ...
            {(["online", "local"] as const).map((item) => {
                ...
                return (
                    <button
                        key={item}
                        type="button"
                        className="inline-flex h-6 items-center gap-1 rounded-full px-2 transition-[background-color,color,transform] duration-200 active:scale-95"
                        style={{ background: active ? theme.node.fill : "transparent", color: active ? theme.node.text : theme.node.muted, boxShadow: active ? `0 4px 12px ${theme.spatial.shadow}` : "none" }}
