            ...
            {section === "all" ? (
                <>
                    ...
                    <CapabilityGroup title="音频" description="音频引用与视频同步音频输出">
                        ...
                        <ParameterField
                            label="音频不得长于输出"
                            description="避免上游静默截断驱动音频"
                            supported={profile.references.audioMustFitOutput === true}
                            disabled={disabled}
                            onChange={(audioMustFitOutput) => updateReferences({ audioMustFitOutput })}
                        />
                        <BooleanField label="同步音频" value={profile.generateAudio} disabled={disabled} onChange={(generateAudio) => update({ generateAudio })} />
                    </CapabilityGroup>
                    ...
                    <CapabilityGroup title="通用限制" description="不属于单一媒体类型的请求与输出约束">
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                            ...
                            <NumberField
                                label="最少视觉素材"
                                value={profile.references.minVisualReferences}
                                min={0}
                                max={profile.references.maxVisualReferences}
                                disabled={disabled}
                                onChange={(value) => updateReferences({ minVisualReferences: value || 0 })}
                            />
                            <NumberField
                                label="最大视觉素材"
                                value={profile.references.maxVisualReferences}
                                min={0}
                                disabled={disabled}
                                onChange={(value) => {
                                    const maxVisualReferences = value || 0;
                                    updateReferences({ maxVisualReferences, minVisualReferences: Math.min(profile.references.minVisualReferences || 0, maxVisualReferences) });
                                }}
                            />
                            <BooleanField label="水印" value={profile.watermark} disabled={disabled} onChange={(watermark) => update({ watermark })} />
                        </div>
