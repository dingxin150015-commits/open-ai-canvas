...
function BooleanField({ label, value, disabled, onChange }: { label: string; value: { supported: boolean; default: boolean }; disabled: boolean; onChange: (value: { supported: boolean; default: boolean }) => void }) {
    return <div className="flex items-center justify-between rounded-md border border-border/60 px-2.5 py-2"><div><div className="text-xs font-medium">{label}</div><div className="text-[var(--fs-tiny)] text-foreground/45">左侧：是否支持 | 右侧：默认值</div></div><div className="flex items-center gap-2"><div className="flex flex-col items-center gap-0.5"><Switch size="small" disabled={disabled} checked={value.supported} onChange={(supported) => onChange({ ...value, supported })} /><span className="text-[10px] text-foreground/50">支持</span></div><div className="flex flex-col items-center gap-0.5"><Switch size="small" disabled={disabled || !value.supported} checked={value.default} onChange={(defaultValue) => onChange({ ...value, default: defaultValue })} /><span className="text-[10px] text-foreground/50">默认</span></div></div></div>;
}
