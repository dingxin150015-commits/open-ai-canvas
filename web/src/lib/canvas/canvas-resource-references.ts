...
function removeCanvasResourceReferenceTokens(prompt: string, references: CanvasResourceReference[]) {
    return compactRemovedCanvasMentionPrompt(
...
function rewriteCanvasPromptAfterReferenceChange(prompt: string, previousReferences: CanvasResourceReference[], nextReferences: CanvasResourceReference[]) {
    ...
    previousCanvasReferences
        .filter((reference) => !nextByNodeId.has(reference.nodeId))
        .forEach((reference) => {
...
function writeCanvasNodePrompt(node: CanvasNodeData, prompt: string) {
    ...
    const promptTemplateMetadata = node.metadata?.promptTemplateOperation ? { promptTemplateOperation: undefined, promptTemplateVariables: undefined } : {};
    return {
        ...node,
        metadata: hasExistingContent ? { ...node.metadata, ...promptTemplateMetadata, composerContent: prompt } : { ...node.metadata, ...promptTemplateMetadata, prompt, composerContent: prompt },
    };
}
