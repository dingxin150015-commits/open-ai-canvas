    ...
    if (asset.mediaType === "video") {
        return (
            <div className="relative overflow-hidden rounded-md bg-black">
                <video src={url ?? undefined} poster={poster ?? undefined} preload="metadata" controls playsInline className="aspect-video w-full" />
...
function AssetDetailSheet({ asset, added, onAdd, onClose }: { asset: ProjectAsset; added: boolean; onAdd: () => void; onClose: () => void }) {
    ...
    return (
        ...
        >
            <div className="flex items-center gap-2 border-b border-[var(--director-sequencer-border)] px-2.5 py-2">
                ...
                <span className="min-w-0 flex-1 truncate text-[11px] font-medium text-[var(--director-dock-fg-strong)]">{asset.title || asset.storageKey}</span>
                <button
                    ref={closeRef}
                    type="button"
                    onClick={onClose}
            ...
            <div className="director-scroll min-h-0 flex-1 overflow-y-auto px-2.5 py-2">
                ...
                {asset.previewText ? <p className="mt-2 line-clamp-2 text-[10px] leading-relaxed text-[var(--director-dock-fg)]/60">{asset.previewText}</p> : null}
            </div>
            <div className="border-t border-[var(--director-sequencer-border)] p-2">
                <button
                    ...
                    className={`flex h-7 w-full items-center justify-center gap-1 rounded-md text-[11px] font-medium transition-colors ${
                        added ? "bg-[var(--director-success-soft)] text-[var(--director-success)]" : "bg-[var(--director-accent)] text-[var(--director-on-accent)] hover:bg-[var(--director-accent-hover)]"
                    }`}
                >
                ...
                {FILTER_TABS.map((tab) => (
                    <button
                        ...
                        className={`h-6 rounded-md px-2 text-[10px] transition-colors ${
                            filter === tab.id ? "bg-[var(--director-dock-active-surface)] text-[var(--director-dock-fg-strong)]" : "text-[var(--director-dock-fg)] hover:bg-[var(--director-control-hover)]"
                        }`}
                    >
                        {tab.label}
                    </button>
            ...
            <div className="relative min-h-0 flex-1">
                <div className="director-scroll h-full overflow-y-auto p-1.5">
                    {assets.length === 0 ? (
                        ...
                        <div className="flex h-full flex-col items-center justify-center gap-2 p-4 text-center">
                            <p className="text-xs text-[var(--director-dock-fg)]/60">{filter === "uploaded" ? "暂无本地上传的媒体" : filter === "project" ? "暂无项目素材" : "项目暂无资产"}</p>
                            <p className="max-w-[180px] text-[11px] leading-relaxed text-[var(--director-dock-fg)]/45">点击上方导入媒体，或将文件拖入此区域</p>
                        </div>
                    ) : (
                ...
                {expandedAsset ? <AssetDetailSheet asset={expandedAsset} added={added === expandedAsset.id} onClose={() => setExpandedId(null)} onAdd={() => addToTimeline(expandedAsset)} /> : null}
            </div>
        </div>
    );
}
