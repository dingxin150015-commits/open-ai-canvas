                        ...
                        <section className="min-w-0">
                            ...
                            {loading ? (
                                ...
                                <div className="eagle-empty-state">
                                    <EmptyState size="compact" title="当前 Eagle 文件夹没有文件" />
                                </div>
                            )}
                        </section>
                    </div>
...
function EagleAssetDrawer({ item, onClose, totalBytes }: { item: ExternalAssetItem | null; onClose: () => void; totalBytes: number }) {
    return (
        <Drawer className="library-drawer" title="素材档案" open={Boolean(item)} size="large" onClose={onClose}>
            {item ? (
