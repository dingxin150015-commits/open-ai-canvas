...
export default function ProjectDetailPage() {
    ...
    const overviewQuery = useQuery({
        ...
        enabled: Boolean(projectId) && activeView === "overview",
    });
    ...
    const createCanvas = async () => {
        ...
        try {
            const [{ createCanvasProjectWithRemoteSync }, storyboard] = await Promise.all([import("@/services/user-data-sync"), unit && shots.length ? import("@/lib/canvas/project-chapter-storyboard") : Promise.resolve(null)]);
...
function ProjectWorkspaceTopBar({
    ...
    return (
        <div className="project-workspace-topbar">
            <button type="button" onClick={() => navigate("/projects")} className="project-workspace-back" aria-label="返回项目" title="返回项目">
                <ArrowLeft />
            </button>
                ...
                {views.map((item) => {
                    ...
                    return (
                        <Tooltip key={item.key} title={item.label} delay={150}>
                            <Link to={href} aria-label={item.label} aria-current={active ? "page" : undefined} className={`project-workspace-tab ${active ? "is-active" : ""}`}>
                                <Icon />
            ...
            {activeView === "workflow" ? (
                ...
                <Tooltip title={createCanvasLabel} delay={150}>
                    <button
                        type="button"
