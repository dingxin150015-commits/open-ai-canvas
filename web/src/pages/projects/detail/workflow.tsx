...
export default function ProjectWorkflowView({ detail, projectId, unitId, stage }: Props) {
    ...
    const refresh = async () => {
        await Promise.all([queryClient.invalidateQueries({ queryKey: ["project", projectId] }), queryClient.invalidateQueries({ queryKey: ["projects"] })]);
    };
    ...
    if (!unit) {
        return (
            <div className="grid h-full place-items-center">
                <Empty description="先添加一个章节，再进入分镜制作">
            ...
            <main className={`workflow-stage-content ${productionStage ? "is-production" : ""}`}>
                {activeStage === "story" ? (
                    <div className="workflow-overview-scroll thin-scrollbar">
                        <StoryStage detail={detail} projectId={projectId} unitId={unit.id} />
                    </div>
                ) : null}
                {activeStage === "assets" ? (
                    <div className="workflow-overview-scroll thin-scrollbar">
                        <AssetsStage detail={detail} projectId={projectId} unitId={unit.id} />
                    </div>
                ) : null}
                {productionStage ? (
                    <Suspense fallback={<div className="workflow-workbench-loading">正在准备分镜工作台…</div>}>
                        <WorkflowProductionWorkbench
                ...
                {activeStage === "delivery" ? (
                    <div className="workflow-overview-scroll thin-scrollbar">
                        <DeliveryStage detail={detail} unitId={unit.id} />
                    </div>
                ) : null}
            </main>
        </div>
