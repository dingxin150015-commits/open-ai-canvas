...
export default function ProjectCanvasesView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    const linksByCanvas = useMemo(
        () =>
            canvasUnitLinks.reduce<Record<string, typeof canvasUnitLinks>>((result, link) => {
    ...
    const canvases = useMemo(
        ...
        [canvasesQuery.data?.canvases, detail.project.id, localCanvases],
    );

    return (
        <div>
            {canvasesQuery.isLoading ? (
                <WorkspaceState icon="canvas" title="正在读取项目画布" description="按页加载画布摘要与章节关联。" />
            ) : canvases.length ? (
                <>
                                    ...
                                    footer={
                                        <div className="border-t border-border/60 pt-2.5">
                                            <div className="flex items-center justify-between">
                                                <span className="text-[var(--fs-tiny)] font-medium text-foreground/48">关联章节</span>
                                                <span className="text-[var(--fs-micro)] tabular-nums text-foreground/38">{linkedUnits.length} 个</span>
                                            </div>
                                            ...
                                            <div className="mt-1.5 flex items-center gap-1.5">
                                                <Select
                                                    ...
                                                    placeholder={unlinkedUnits.length ? "关联更多章节" : "全部章节已关联"}
                                                ...
                                                <Popconfirm
                                                    title="解除画布与项目的关系？"
                                                    description="画布文档不会删除，之后仍可在“画布”中打开。"
                                                    okText="解除关系"
                                                    cancelText="取消"
                                                ...
                                                >
                                                    <Tooltip title="解除项目关系">
                                                        <Button size="small" type="text" danger icon={<Unlink className="size-3.5" />} aria-label="解除项目关系" />
                                                    </Tooltip>
                                                </Popconfirm>
                                            </div>
                                        </div>
                    ...
                    <PaginationBar
                        current={page}
                        pageSize={pageSize}
                        total={canvasesQuery.data?.total || 0}
                        itemLabel="张"
