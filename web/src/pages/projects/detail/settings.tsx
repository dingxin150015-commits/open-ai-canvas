...
export default function ProjectSettingsView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    useEffect(() => {
        ...
        setDefaultImageModel(project.defaultImageModel || "");
        setDefaultVideoModel(project.defaultVideoModel || "");
    }, [project]);
    const dirty = useMemo(
        () =>
            ...
            defaultImageModel !== (project.defaultImageModel || "") ||
            defaultVideoModel !== (project.defaultVideoModel || ""),
        [aspectRatio, defaultImageModel, defaultVideoModel, description, name, project, sourceType, stylePresetId, styleProfileJson],
    );
    ...
    const coverResourceByItemId = useMemo(
        () =>
            new Map(
                coverPickerItems.flatMap((item) => {
    ...
    const saveMutation = useMutation({
        mutationFn: () => updateProject(project.id, { name: name.trim(), description, aspectRatio, sourceType, stylePresetId, styleProfileJson, defaultImageModel, defaultVideoModel }),
        onSuccess: () => {
            refreshProject();
    ...
    const archiveMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            refreshProject();
            message.success(project.status === "archived" ? "项目已恢复" : "项目已归档");
        },
        onError: (error) => message.error(error instanceof Error ? error.message : "项目状态更新失败"),
    });
    const coverMutation = useMutation({
        mutationFn: (coverResourceId: string) => updateProject(project.id, { coverResourceId }),
        onSuccess: (_, coverResourceId) => {
            setCoverPickerOpen(false);
            refreshProject();
            message.success(coverResourceId ? "项目主图已更新" : "项目主图已移除");
        },
        onError: (error) => message.error(error instanceof Error ? error.message : "项目主图更新失败"),
    });
        ...
        <div>
            <header className="flex items-end justify-between gap-3 pb-3">
                <div>
                    ...
                    <p className="mt-1 text-xs text-foreground/48">基础信息、项目画风与归档管理</p>
                </div>
            ...
            <section className="py-5">
                ...
                <div className="grid gap-x-4 gap-y-3 md:grid-cols-2 xl:grid-cols-4">
                    <Field label="项目名称" className="xl:col-span-2">
                    ...
                    <Field label="项目简介" className="md:col-span-2 xl:col-span-4">
                        <Input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="一句话说明项目目标" />
                    </Field>
                </div>
            </section>
            ...
            <section className="border-t border-border/70 py-5">
                <div className="mb-3">
                    <h3 className="text-sm font-semibold">默认生成模型</h3>
                    <p className="mt-0.5 text-[var(--fs-label)] text-foreground/45">分镜图、动作预演与镜头视频优先使用项目默认；未设置或模型不可用时跟随工作台全局默认。</p>
                </div>
                    ...
                    <Field label="默认生图模型">
                        <div className="flex items-center gap-2">
                            <ModelPicker
                                ...
                                fullWidth
                                placeholder={`跟随全局 · ${modelDisplayName(effectiveConfig, effectiveConfig.imageModel) || "未配置"}`}
                                showSelectedPrice
                            />
                            {defaultImageModel ? (
                                <Button type="text" size="small" onClick={() => setDefaultImageModel("")}>
                                    跟随全局
                                </Button>
                            ) : null}
                        </div>
                    ...
                    <Field label="默认视频模型">
                        <div className="flex items-center gap-2">
                            <ModelPicker
                                ...
                                fullWidth
                                placeholder={`跟随全局 · ${modelDisplayName(effectiveConfig, effectiveConfig.videoModel) || "未配置"}`}
                                showSelectedPrice
                            />
                            {defaultVideoModel ? (
                                <Button type="text" size="small" onClick={() => setDefaultVideoModel("")}>
                                    跟随全局
                                </Button>
                            ) : null}
                        </div>
            ...
            <section className="border-t border-border/70 py-5">
                <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                        <h3 className="text-sm font-semibold">项目主图</h3>
                        <p className="mt-0.5 text-[var(--fs-label)] text-foreground/45">项目列表优先展示这张图片；未设置时使用项目画风示意图</p>
                    </div>
                    <span className="text-[var(--fs-label)] text-foreground/40">建议使用与项目画幅一致的图片</span>
                </div>
                <div className="flex flex-col gap-3 rounded-lg border border-border/70 bg-surface-active p-3 sm:flex-row sm:items-center">
                    {project.coverResourceId ? (
                        <img src={resourceFileUrl(project.coverResourceId)} alt={`${project.name}项目主图`} className="aspect-video w-full shrink-0 rounded-md bg-foreground/5 object-cover sm:w-52" />
                    ) : (
                        <span className="grid aspect-video w-full shrink-0 place-items-center rounded-md bg-foreground/5 text-foreground/30 sm:w-52">
                            <ImageIcon className="size-6" />
                        </span>
                    )}
                    <div className="min-w-0 flex-1">
                        <div className="text-sm font-medium">{project.coverResourceId ? "已设置项目主图" : "尚未设置项目主图"}</div>
                        <p className="mt-1 text-xs leading-5 text-foreground/48">从个人素材库或项目素材库选择，也可以在选择窗口中上传一张新图片。</p>
                    </div>
                    <div className="flex shrink-0 gap-2">
                        ...
                        >
                            {project.coverResourceId ? "替换主图" : "设置主图"}
                        </Button>
                        {project.coverResourceId ? (
                            <Button danger type="text" icon={<Trash2 className="size-3.5" />} loading={coverMutation.isPending} onClick={() => coverMutation.mutate("")}>
                                移除
                            </Button>
                        ) : null}
                    </div>
            ...
            <section className="border-t border-border/70 py-5">
                <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                        <h3 className="text-sm font-semibold">项目画风</h3>
                        <p className="mt-0.5 text-[var(--fs-label)] text-foreground/45">项目保存当前版本快照；修改“我的风格”不会自动改写历史项目</p>
                    </div>
                    {styleProfile ? (
                        <span className="text-[var(--fs-label)] text-foreground/52">{styleProfile.source === "user" ? "来自我的风格" : styleProfile.source === "external" ? "外部导入" : "系统预设"}</span>
                    ) : (
                        <span className="text-[var(--fs-label)] text-foreground/40">未设置</span>
                    )}
                </div>
                <div className="flex flex-col gap-3 rounded-lg bg-surface-active p-3 lg:flex-row lg:items-center">
                    {selectedStyle ? (
                        <img src={selectedStyle.imageUrl} width="160" height="90" alt={`${selectedStyle.title}画风示意`} className="aspect-video w-40 shrink-0 rounded-md object-cover" />
                    ) : (
                    ...
                    <div className="flex shrink-0 flex-wrap gap-2">
                        ...
                        >
                            {selectedStyle ? "更换画风" : "选择画风"}
                        </Button>
                    </div>
                </div>
                {styleProfile ? (
                    <div className="mt-2 grid grid-cols-2 gap-2 text-xs sm:grid-cols-5">
                        <StyleMetric label="执行策略" value={styleProfile.executionPolicy === "strict-assets" ? "严格校验" : "兼容降级"} />
                        <StyleMetric label="绑定资产" value={`${styleProfile.assets.length} 个`} />
                        <StyleMetric label="已启用" value={`${enabledStyleAssets.length} 个`} />
                        <StyleMetric label="图片执行" value={styleExecutionStatusLabel(styleExecutionPlans?.image.status)} />
                        <StyleMetric label="视频执行" value={styleExecutionStatusLabel(styleExecutionPlans?.video.status)} />
                    </div>
                ) : null}
                {styleExecutionPlans && (styleExecutionPlans.image.warnings.length || styleExecutionPlans.video.warnings.length) ? (
                    <div className="mt-2 grid gap-1 rounded-md bg-amber-500/5 px-3 py-2 text-[var(--fs-label)] leading-5 text-amber-600 dark:text-amber-400">
                        {styleExecutionPlans.image.warnings.length ? <p>图片：{styleExecutionPlans.image.warnings.join("；")}</p> : null}
                        {styleExecutionPlans.video.warnings.length ? <p>视频：{styleExecutionPlans.video.warnings.join("；")}</p> : null}
                    </div>
                ) : null}
            </section>
                ...
                <div className="flex flex-col gap-3 rounded-lg bg-red-500/5 px-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex min-w-0 items-center gap-2.5">
                        ...
                        <div className="min-w-0">
                            <h3 className="text-sm font-medium">{project.status === "archived" ? "恢复项目" : "归档项目"}</h3>
                            <p className="mt-0.5 text-[var(--fs-label)] text-foreground/48">{project.status === "archived" ? "恢复后可继续创建章节、画布和生成任务" : "保留全部章节、画布和资产，停止项目内新建与生成"}</p>
                        </div>
                    </div>
            ...
            <Modal
                ...
                open={archiveOpen}
                okText={project.status === "archived" ? "确认恢复" : "确认归档"}
                cancelText="取消"
            ...
            >
                <p className="m-0 text-sm leading-6 text-foreground/65">{project.status === "archived" ? "恢复后项目会重新进入可编辑状态。" : "归档不会删除章节、画布或资产，画布文档仍可在创作画布中打开。"}</p>
            </Modal>
            <AssetLibraryPickerModal
                ...
                pagination={{
                    ...
                    total: coverAssetsQuery.data?.total || 0,
                    onChange: (nextPage, nextPageSize) => {
                        setCoverPage(nextPageSize !== coverPageSize ? 1 : nextPage);
                        setCoverPageSize(nextPageSize);
                    },
                }}
                upload={{
                    accept: "image/png,image/jpeg,image/webp,image/avif",
                    description: "支持 PNG、JPG、WebP、AVIF",
