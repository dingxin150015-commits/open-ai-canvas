    ...
    return (
        <section className="mx-auto max-w-5xl">
            ...
            <div className="mt-6 rounded-xl border border-border/70 bg-surface p-5">
                ...
                <div className="max-h-[60vh] whitespace-pre-wrap text-sm leading-7 text-foreground/78">{unit.sourceText || "当前章节还没有正文。请先在剧情章节中上传小说或添加内容。"}</div>
            </div>
        </section>
    );
}
...
export function AssetsStage({ detail, projectId, unitId }: { detail: ProjectDetail; projectId: string; unitId: string }) {
    ...
    return (
        <section className="mx-auto max-w-6xl">
            <StageHeading eyebrow="02 / 资产拆分" title="确认镜头真正会使用的资产" description="角色、场景、道具、素材与其他资产先建立稳定版本，镜头再绑定具体版本。" />
            ...
            <div className="mt-5 flex items-center justify-between border-y border-border/70 py-5">
                <div>
...
export function DeliveryStage({ detail, unitId }: { detail: ProjectDetail; unitId: string }) {
    ...
    return (
        <section className="mx-auto max-w-5xl">
            <StageHeading eyebrow="06 / 交付与打包" title="交付前质量门禁" description="所有镜头视频就绪、过期产物清零后，再打包成片与生产资料。" />
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
                <MetricCard icon={<Film className="size-5" />} label="视频已就绪" value={`${readyVideos.length} / ${shots.length}`} />
                <MetricCard icon={<Clock3 className="size-5" />} label="总时长" value={formatDuration(shots.reduce((total, item) => total + item.durationMs, 0))} />
            ...
            <div className="mt-5 border-y border-border/70 py-5">
                <div className="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-xs leading-5 text-foreground/48">成片 MP4、字幕 SRT、分镜 JSON/CSV、资产清单和生成参数 ZIP。</p>
                    </div>
                </div>
