...
export default function ProjectChaptersView({ detail, refreshProject }: ProjectDetailViewProps) {
    ...
    const charactersGenerated = Boolean(
        ...
        (completedChapterOperations[chapterOperationKey(selectedUnit.id, "characters")] ||
            ...
            detail.assetCandidates.some((candidate) => candidate.unitId === selectedUnit.id && candidate.category === "character")),
    );
    ...
    return (
        <div className="grid h-full min-h-0 min-w-0 w-full grid-rows-[minmax(180px,34vh)_minmax(0,1fr)] overflow-hidden lg:grid-cols-[280px_minmax(0,1fr)] lg:grid-rows-1">
            <aside className="flex min-h-0 min-w-0 w-full flex-col border-b border-border/70 bg-background/28 lg:border-b-0 lg:border-r">
                <div className="flex h-11 shrink-0 items-center justify-between border-b border-border/70 px-2.5">
                    <div className="text-xs font-medium text-foreground/55">
                        章节 <span className="ml-1 tabular-nums text-foreground/35">{detail.units.length.toLocaleString("zh-CN")}</span>
                    </div>
                    <div className="flex items-center gap-0.5">
                        ...
                        <Tooltip title="导入小说">
                            <Button type="text" size="small" icon={<FileUp className="size-3.5" />} aria-label="导入小说" onClick={() => setImportOpen(true)} />
                        </Tooltip>
                        <Tooltip title="添加章节">
                            <Button type="text" size="small" icon={<Plus className="size-4" />} aria-label="添加章节" onClick={() => setCreateOpen(true)} />
                        </Tooltip>
                    </div>
                </div>
                                    ...
                                    return (
                                        <div key={unit.id} className="absolute left-0 top-0 w-full" style={{ height: virtualItem.size, transform: `translateY(${virtualItem.start}px)` }}>
                                            <div
                                                ...
                                                className={`group relative flex h-[58px] items-start rounded-lg border px-0.5 transition-colors duration-100 ${unit.id === selectedUnit?.id ? "border-border/90 bg-surface-active" : "border-transparent hover:border-border/60 hover:bg-surface-hover"} ${draggedId === unit.id ? "opacity-45" : ""}`}
                                            >
                                                <button
                                                    type="button"
                        ...
                        <header className="flex shrink-0 flex-wrap items-start gap-3 border-b border-border/70 px-4 py-3">
                            <div className="min-w-0 flex-1">
                                ...
                                <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-[var(--fs-tiny)] text-foreground/38">
                                    <span>{dirty ? "有未保存修改" : `保存于 ${formatTime(selectedUnit.updatedAt)}`}</span>
                                    <span>·</span>
                            ...
                            <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
                                <Button
                                    ...
                                    disabled={!selectedUnit || dirty || Boolean(characterOperation)}
                                    loading={Boolean(characterOperation)}
                                    onClick={() => {
                                    ...
                                    aria-label={characterOperation ? `提取角色，已运行 ${formatOperationElapsed(characterOperation.startedAt, operationNow)}` : "提取角色"}
                                >
                                    {characterOperation ? `提取角色（已运行${formatOperationElapsed(characterOperation.startedAt, operationNow)}）` : charactersGenerated ? "提取角色（已生成）" : "提取角色"}
                                </Button>
                                <Button
                                    size="small"
                                    type="primary"
            ...
            <Modal
                ...
                onCancel={() => setCharacterExtractOpen(false)}
                onOk={() => void extractCharacters()}
            ...
            <Modal
                ...
                cancelText="取消"
                okButtonProps={{ disabled: !selectedTextModel }}
                onCancel={() => setStoryboardOpen(false)}
                onOk={() => void createStoryboard()}
                styles={{ body: { paddingTop: 12 } }}
            >
                <div className="grid gap-4">
                    <div className="rounded-lg border border-border/70 bg-foreground/[.018] px-3 py-2.5">
                    ...
                    {storyboardImpact.shotCount ? (
                        <Callout tone="warning" title={`本章已有 ${storyboardImpact.shotCount} 个分镜`}>
                            继续后会先生成新分镜；生成成功后，再按确认内容整体替换旧镜头及其关联数据。
                        </Callout>
                    ) : null}
                    <label className="block">
                        <span className="mb-1.5 block text-xs font-medium text-foreground/68">文本模型</span>
            ...
            <Modal
                ...
                okText="移动"
                cancelText="取消"
                okButtonProps={{ disabled: !movePosition || movePosition < 1 || movePosition > orderedUnits.length, loading: reorderMutation.isPending }}
                onCancel={() => {
                    setMoveTargetId("");
                    setMovePosition(null);
                }}
                onOk={moveChapterToPosition}
                styles={{ body: { paddingTop: 12 } }}
            >
                ...
                <label className="mt-3 flex items-center gap-2 text-sm">
                    ...
                    <InputNumber min={1} max={orderedUnits.length} precision={0} value={movePosition} onChange={setMovePosition} className="min-w-0 flex-1" />
...
function CreateChapterModal({ open, onClose, loading, onSubmit }: { open: boolean; onClose: () => void; loading: boolean; onSubmit: (values: { title: string; sourceText?: string }) => void }) {
    return (
        <Modal title="添加章节" open={open} footer={null} destroyOnHidden onCancel={onClose} width={480} styles={{ body: { paddingTop: 12 } }}>
            <Form layout="vertical" onFinish={onSubmit}>
                ...
                <Form.Item name="sourceText" label="正文（可选）">
                    <Input.TextArea rows={4} placeholder="创建后仍可继续编辑和排版" />
                </Form.Item>
        ...
        <Modal title={null} open={open} footer={null} destroyOnHidden onCancel={onClose} width={760} styles={{ container: { padding: 0, overflow: "hidden" }, body: { padding: 0 } }}>
            <div className="flex min-h-[478px] flex-col">
                <header className="flex h-12 shrink-0 items-center border-b border-border px-4">
                    <div>
                        <h2 className="text-sm font-semibold">导入小说</h2>
                        <p className="mt-0.5 text-[var(--fs-tiny)] text-foreground/42">自动识别章节标题，确认后追加到当前项目</p>
                    </div>
                </header>
                <div className="grid min-h-[430px] flex-1 grid-cols-1 md:grid-cols-[minmax(0,1fr)_240px]">
                    <div className="border-b border-border p-3 md:border-b-0 md:border-r">
                        <div className="mb-2 flex items-center justify-between gap-2">
                            <div>
                                <div className="text-sm font-medium">小说正文</div>
                                <div className="mt-0.5 text-[var(--fs-label)] text-foreground/45">识别章节标题后追加到现有章节</div>
                            </div>
                    ...
                    <div className="flex min-h-0 flex-col">
                        <div className="flex h-11 shrink-0 items-center justify-between border-b border-border px-3 text-xs">
                            <span className="font-medium">拆分预览</span>
                            <span className="tabular-nums text-foreground/45">{chapters.length} 章</span>
                        </div>
                        ...
                        <div className="flex shrink-0 items-center justify-between gap-2 border-t border-border p-3">
                            <span className={`text-[var(--fs-tiny)] ${chapters.length > MAX_NOVEL_IMPORT_CHAPTERS ? "text-red-500" : "text-foreground/38"}`}>
                                {chapters.length > MAX_NOVEL_IMPORT_CHAPTERS ? `最多一次导入 ${MAX_NOVEL_IMPORT_CHAPTERS.toLocaleString("zh-CN")} 章` : `支持最多 ${MAX_NOVEL_IMPORT_CHAPTERS.toLocaleString("zh-CN")} 章`}
                            </span>
                            <div className="flex gap-2">
                                <Button size="small" onClick={onClose}>
                                    取消
                                </Button>
                                <Button size="small" type="primary" disabled={!chapters.length || chapters.length > MAX_NOVEL_IMPORT_CHAPTERS} loading={loading} onClick={() => onImport(chapters)}>
                                    导入 {chapters.length || ""} 章
                                </Button>
                            </div>
                        </div>
                    </div>
            ...
            {chapters.length ? (
                <div className="relative w-full" style={{ height: virtualizer.getTotalSize() }}>
                    {virtualizer.getVirtualItems().map((virtualItem) => {
                        ...
                        return (
                            <div key={`${chapter.title}-${virtualItem.index}`} className="absolute left-0 top-0 flex w-full gap-2 border-b border-border/60 px-1.5 py-2" style={{ height: virtualItem.size, transform: `translateY(${virtualItem.start}px)` }}>
