...
export default function AssetsPage() {
    ...
    const visibleAssets = useMemo(() => (assetPageQuery.data?.assets || localVisibleAssets).map(normalizeAssetRecord).filter((asset): asset is LibraryAsset => asset.kind !== "entity"), [assetPageQuery.data?.assets, localVisibleAssets]);
    ...
    const categoryCounts = useMemo(
        () => assetCountMap(categoryOptions, assetPageQuery.data?.categoryCounts, viewMode === "trash" ? trashAssets : activeAssets, (asset) => asset.category || "other"),
        [activeAssets, assetPageQuery.data?.categoryCounts, trashAssets, viewMode],
    );
                ...
                <div className="studio-band">
                    ...
                    >
                        ...
                        <Select
                            ...
                            options={[
                                { label: "舒适 · 6 列", value: 6 },
                                { label: "标准 · 8 列", value: 8 },
                                { label: "紧凑 · 10 列", value: 10 },
                            ]}
                        ...
                        <aside className="thin-scrollbar flex gap-2 overflow-x-auto py-3 lg:sticky lg:top-0 lg:block lg:max-h-[calc(100vh-150px)] lg:overflow-y-auto lg:pr-3">
                            ...
                            <div className="mt-5">
                                ...
                                <div className="space-y-0.5">
                                    <button
                                        type="button"
                                        aria-pressed={folderFilter === "uncategorized"}
                                        className={`assets-filter-item w-full ${folderFilter === "uncategorized" ? "is-active" : ""}`}
                                        onClick={() => {
                                            setFolderFilter("uncategorized");
                                            setPage(1);
                                        }}
                                    >
                                        <span className="assets-filter-item-label">未分类</span>
                                        <span className="assets-filter-count">{folderCounts[""] ?? activeAssets.filter((asset) => !asset.folderId).length}</span>
                                    </button>
                                    {folders.map((folder) => (
                                        <div key={folder.id} className="assets-folder-row">
                                            <button
                                                ...
                                                className={`assets-filter-item min-w-0 flex-1 ${folderFilter === folder.id ? "is-active" : ""}`}
                                                onClick={() => {
                                                    setFolderFilter(folder.id);
                                                    setPage(1);
                                                }}
                                            >
                                                <span className="assets-filter-item-label min-w-0 truncate">{folder.name}</span>
                                                <span className="assets-filter-count">{folderCounts[folder.id] ?? activeAssets.filter((asset) => asset.folderId === folder.id).length}</span>
                                            </button>
                                            <Dropdown
                                                trigger={["click"]}
                                                menu={{
                                                    items: [
                                                        ...
                                                        { key: "delete", danger: true, icon: <Trash2 className="size-3.5" />, label: "删除分类", onClick: () => void removeFolder(folder) },
                                                    ],
                                                }}
                                            >
                                                <button type="button" className="assets-folder-more" aria-label={`管理分类 ${folder.name}`} title="管理分类">
