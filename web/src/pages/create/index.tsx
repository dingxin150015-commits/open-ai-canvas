...
export default function CreatePage() {
        ...
        () => ({
            ...
            options:
                mode === "image"
                    ...
                    : mode === "video"
                      ? {
                            ...(preferredVideoProfile.ratios.length > 0 ? { size: ratio } : {}),
                            videoSeconds: Number(seconds),
    ...
    const settingsDraftIdentity = useMemo<CreationSettingsDraftIdentity | undefined>(
        () =>
            mode === "text"
                ? undefined
                : {
    ...
    useEffect(() => {
        ...
        if (!composerPreferencesHydrated || !composerPreferencesInitialized) {
        ...
        setSettingsReady(false);
        let cancelled = false;
        void loadCreationSettingsDraft(settingsDraftIdentity, creationSettingsScope)
            .catch((error) => {
            ...
            .then((draft) => {
                ...
                if (mode === "image") {
                    ...
                    const normalized = normalizeImageValue(imageProfile, {
                        size: draft?.ratio || saved?.ratio || imageProfile.size.default,
                        quality: draft?.quality || saved?.quality || imageProfile.quality.default,
                        count: draft?.count || saved?.count || String(Math.min(imageProfile.maxOutputs, Number(count) || 1)),
                    });
                    ...
                    const normalized = normalizeVideoValue(videoProfile, {
                        seconds: draft?.seconds || saved?.seconds || String(videoProfile.duration.default),
                        ratio: draft?.ratio || saved?.ratio || videoProfile.defaultRatio,
                        resolution: draft?.videoQuality || saved?.videoQuality || videoProfile.defaultResolution,
                    });
    ...
    }, [composerPreferencesHydrated, composerPreferencesInitialized, creationSettingsScope, imageProfile, mode, settingsDraftIdentity, settingsDraftPersistenceKey, videoProfile]);

    useEffect(() => {
        if (!settingsDraftIdentity || settingsDraftReadyKeyRef.current !== settingsDraftPersistenceKey) return;
        const draft = mode === "image" ? { ratio, quality, count } : { ratio, seconds, videoQuality, videoGenerateAudio, videoWatermark };
    ...
    const submit = async (retryContext?: CreationRetryContext, retryLockKey?: string) => {
        ...
        const assistantMessage = newMessage("assistant", "", {
            mode,
            model: selectedModel,
            status: mode === "text" && textStreaming ? "streaming" : "pending",
            settings,
            ...retryContext,
        });
                    ...
                    <AssetLibraryPickerModal
                        ...
                        upload={{
                            ...
                            external: {
                                ...
                                onUpload: (files, folderId) => externalAssetSources.uploadExternalFiles(files, folderId),
                            },
                        }}
...
function CreationComposer(props: ComposerProps) {
    ...
    const canSubmit = Boolean(props.prompt.trim()) && !interactionBusy && props.settingsReady;
    ...
    const actionLabel = !props.settingsReady ? "正在加载模型设置" : props.referenceReplacementBusy ? "正在替换参考图" : interactionBusy || (props.generationActive && !canSubmit) ? "生成中" : showCost ? `预计消耗 ${formattedCredits} 积分，发送` : "发送";
        ...
        <div className="creation-parameter-menu">
            ...
            {props.mode === "video" ? (
                <>
                    ...
                    {props.videoProfile.generateAudio.supported || props.videoProfile.watermark.supported ? (
                        <SettingSection
                            title="输出"
                            value={[...(props.videoProfile.generateAudio.supported ? [videoGenerateAudio ? "有声" : "无声"] : []), ...(props.videoProfile.watermark.supported ? [videoWatermark ? "有水印" : "无水印"] : [])].join(" · ")}
                        >
                            <div className="creation-output-switches">
                                {props.videoProfile.generateAudio.supported ? (
                                    <div className="creation-output-switch">
                                        <span>
                                            <strong>生成声音</strong>
                                            <small>{videoGenerateAudio ? "视频将包含模型生成的声音" : "生成无声视频"}</small>
                                        </span>
                                        <Switch size="sm" checked={videoGenerateAudio} onChange={props.onVideoGenerateAudioChange} aria-label="生成声音" />
                                    </div>
                                ) : null}
                                {props.videoProfile.watermark.supported ? (
                                    <div className="creation-output-switch">
                                        <span>
                                            <strong>添加水印</strong>
                                            <small>{videoWatermark ? "在生成结果中添加供应商水印" : "不添加供应商水印"}</small>
                                        </span>
                                        <Switch size="sm" checked={videoWatermark} onChange={props.onVideoWatermarkChange} aria-label="添加水印" />
                                    </div>
                                ) : null}
                            </div>
                        </SettingSection>
...
function DurationMenu({ profile, seconds, onChange }: { profile: VideoCapabilityConfig; seconds: string; onChange: (value: string) => void }) {
    ...
    const rangeValue = value === -1 ? profile.duration.default : value;
    const durationControl = (
        <>
            {profile.duration.smartSupported ? (
                <div className="creation-duration-choices">
                    <button type="button" className={value === -1 ? "is-selected" : ""} onClick={() => onChange("-1")}>
                        智能时长
                    </button>
                </div>
            ) : null}
            {profile.duration.selection === "range" ? (
                <>
                    <input className="h-8 w-full" style={{ accentColor: "var(--creation-text)" }} type="range" min={min} max={max} step={step} value={rangeValue} aria-label="视频时长（秒）" onChange={(event) => onChange(event.target.value)} />
                    ...
                    <label className="creation-custom-value is-duration">
                        ...
                        <span className="creation-duration-custom-field">
                            <input
                                ...
                                value={rangeValue}
                                onFocus={(event) => event.currentTarget.select()}
                                onBlur={() => onChange(String(rangeValue))}
    ...
    const durationLabel = value === -1 ? "智能时长" : `${value} 秒`;
    return (
        <Popover
            open={open}
            onOpenChange={setOpen}
        ...
        >
            <button type="button" className="creation-chat-control is-duration" aria-label={`视频时长：${durationLabel}`}>
                <Clock3 />
                <span>{value === -1 ? "智能" : `${value}s`}</span>
                <ChevronDown className={open ? "is-open" : ""} />
            </button>
        </Popover>
    );
...
function thinkingFor(mode: CreationMode, brandName: string): CreationThinking {
    if (mode === "image")
        return {
            title: "正在为你画这一镜",
            hint: `${brandName}正在理解你的构图意图，并把画面交给模型出图。`,
            steps: ["理解构图", "定调画风", "生成画面"],
            activity: "正在理解构图并把画面交给模型出图",
        };
    if (mode === "text")
        return {
            title: "正在为你写这段",
            hint: `${brandName}正在梳理你的创作脉络，组织语言与结构。`,
            steps: ["梳理脉络", "组织语言", "输出段落"],
            activity: "正在梳理脉络并组织语言",
        };
    return {
        title: "正在为你拍这一镜",
        hint: `${brandName}正在拆解你的镜头脚本，设计运镜与光线，并交给模型渲染成片。`,
        steps: ["拆解镜头", "设计运镜", "定调布光", "渲染成片"],
        activity: "正在按导演思路拆解镜头并渲染",
    };
}
        ...
        const messages = conversation.messages.map((message) => {
            ...
            if (matches.every((task) => task.status === "cancelled")) {
                ...
                return {
                    ...
                    content: localOnly ? runtime.localDreaminaCancellationMessage(localOnly) : "已停止",
                    error: undefined,
                    taskIds: nextTaskIds,
                };
            }
            ...
            return {
                ...
                content: "生成失败",
                error: generationErrorMessage(failed?.creationError || failed?.error || "任务已结束，但生成结果暂时无法读取"),
                taskIds: nextTaskIds,
            };
        });
