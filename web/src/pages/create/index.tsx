...
import { backendModelRuntimeRequired, isGenerationTaskCancelled, logicalModelIDForConfig, runBackendGenerationTask, runBackendGenerationTaskBatch, type BackendGenerationResult } from "@/services/api/generation-task";
...
function CreationComposer(props: ComposerProps) {
    ...
    const actionLabel = !props.settingsReady ? "正在加载模型设置" : props.referenceReplacementBusy ? "正在替换参考图" : props.busy ? "生成中" : showCost ? `预计消耗 ${formattedCredits} 积分，发送` : "发送";
            ...
            <footer className="creation-chat-dock">
                <div className="creation-chat-controls">
                    ...
                    <Tooltip title="从本机上传附件">
                        <button type="button" className="creation-chat-control" onClick={() => props.fileInputRef.current?.click()} disabled={interactionBusy || !referencesSupported} aria-label="从本机上传附件">
                            <Paperclip />
                            <span>附件</span>
                        </button>
                    </Tooltip>
                    <Tooltip title={!referencesSupported ? "当前模型不支持参考媒体" : "从素材库选择参考内容"}>
                        <button type="button" className="creation-chat-control" onClick={props.onOpenLibrary} disabled={interactionBusy || !referencesSupported} aria-label="打开素材库选择参考内容">
                            <FolderOpen />
                            <span>素材库</span>
                        </button>
                    </Tooltip>
                            ...
                            <div className="creation-output-switches">
                                {props.videoProfile.generateAudio.supported ? (
                                    <div className="creation-output-switch">
                                        ...
                                        <Switch size="small" checked={videoGenerateAudio} onChange={props.onVideoGenerateAudioChange} aria-label="生成声音" />
                                    </div>
                                ) : null}
                                {props.videoProfile.watermark.supported ? (
                                    <div className="creation-output-switch">
                                        ...
                                        <Switch size="small" checked={videoWatermark} onChange={props.onVideoWatermarkChange} aria-label="添加水印" />
                                    </div>
                                ) : null}
                            </div>
                        </SettingSection>
