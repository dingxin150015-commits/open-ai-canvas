...
export function useCanvasNodeOperations({
    ...
    const cloneDrawingForNode = useCallback(
        (source: CanvasNodeData, target: CanvasNodeData, failureMessage: string) => {
            ...
            void cloneCanvasDrawing(projectId, sourceDrawingId, targetDrawingId)
                .then((saved) => {
                    if (!saved) {
    ...
    const createNode = useCallback(
        (type: CanvasNodeTypeId, position?: Position, workflowProvider?: "runninghub" | "comfyui") => {
            ...
            const selectedWorkflowProvider =
                type === CanvasNodeType.Config ? workflowProvider || (workflowProviderPluginEnabled(runtimeStatuses, "runninghub") ? "runninghub" : workflowProviderPluginEnabled(runtimeStatuses, "comfyui") ? "comfyui" : undefined) : undefined;
        ...
        [
            ...
            effectiveConfig.comfyBridge.workflows.length,
            effectiveConfig.runningHub.enabled,
            effectiveConfig.runningHub.workflows.length,
            getCanvasCenter,
            message,
            nodesRef,
            runtimeStatuses,
    ...
    const createFolder = useCallback(
        (position?: Position, linked?: { id: string; projectId: string; title: string; style: CanvasFolderStyle; theme: CanvasFolderTheme; createdAt: string }) => {
    ...
    const createStoryboardGroup = useCallback(() => {
        ...
        const frame = createCanvasNode(
            ...
            { x: left + frameWidth / 2 - padding, y: top + frameHeight / 2 - FRAME_HEADER_HEIGHT - padding },
            {
    ...
    const createReferenceGroup = useCallback(() => {
        ...
        const frame = createCanvasNode(
            CanvasNodeType.Frame,
            { x: left + frameWidth / 2, y: top + frameHeight / 2 },
            {
    ...
    const duplicateNode = useCallback(
        (nodeId: string, duplicateMode: "variant" | "copy" = "variant") => {
            ...
            const copyTitle =
                duplicateMode === "copy"
                    ? nextCopiedNodeTitle(
                          source.title,
                          nodesRef.current.map((node) => node.title),
                      )
                    : undefined;
            ...
            copiedNodes
                .filter((node) => node.type === CanvasNodeType.Drawing)
                .forEach((targetNode) => {
    ...
    const setPrimaryVersion = useCallback(
        (nodeId: string) => {
            const target = nodesRef.current.find((node) => node.id === nodeId);
            if (!target) return;
