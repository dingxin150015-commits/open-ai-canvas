...
export const CanvasProjectWorldLayers = memo(function CanvasProjectWorldLayers(props: CanvasProjectWorldLayersProps) {
    ...
    const batchPreviews = useMemo(
        () =>
            new Map(
                props.visibleNodes
                    .filter((node) => node.metadata?.isBatchRoot)
                    .map((node) => [
                        node.id,
                        (node.metadata?.batchChildIds || [])
                            .filter((id) => id !== node.metadata?.primaryImageId)
    ...
    return (
        <>
            <svg
                ...
                style={{
                    left: props.connectionLayerBounds.left,
                    top: props.connectionLayerBounds.top,
                    width: props.connectionLayerBounds.width,
                    height: props.connectionLayerBounds.height,
                    pointerEvents: "none",
                    zIndex: 0,
                }}
            >
