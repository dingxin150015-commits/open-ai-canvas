...
export function useCanvasSelectionController({
    ...
    const handleNodeMouseDown = useCallback(
        (event: ReactMouseEvent | ReactPointerEvent, nodeId: string) => {
            ...
            dragRef.current = {
                isDraggingNode: true,
                hasMoved: false,
                openPanelOnClick: !isMultiSelectClick,
                startX: event.clientX,
                startY: event.clientY,
                draggedNodeIds,
                draggedRenderNodeIds,
                draggedRenderNodeIdSet,
                initialSelectedNodes,
            };
    ...
    const finishNodeDrag = useCallback(
        (clientX?: number, clientY?: number) => {
            ...
            const rawOffset = {
                x: clientX == null ? 0 : (clientX - dragRef.current.startX) / currentViewport.k,
                y: clientY == null ? 0 : (clientY - dragRef.current.startY) / currentViewport.k,
            };
