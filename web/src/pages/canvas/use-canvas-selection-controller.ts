...
export function useCanvasSelectionController({
    ...
    const handleCanvasMouseDown = useCallback(
        (event: ReactPointerEvent<HTMLDivElement>) => {
            ...
            selectionSpatialIndexRef.current = buildCanvasSpatialIndex(
                currentNodes.filter((node) => !hiddenBatchChildIds.has(node.id) && !(node.parentId && nodeById.get(node.parentId)?.metadata?.frame?.collapsed)).map((node) => ({ id: node.id, bounds: canvasNodeBounds(node), value: node })),
            );
            setSelectedConnectionId(null);
        },
