...
import {
    ...
    type ChannelModelSupportStatus,
...
export function ChannelModelManager({ channel, onClose, onChanged }: { channel: ModelChannel; onClose: () => void; onChanged: () => void | Promise<void> }) {
    ...
    const testModel = async () => {
        ...
        try {
            ...
            const successMsg = result.note ? `模型测试通过，耗时 ${duration} 秒（${result.note}）` : `模型测试通过，耗时 ${duration} 秒`;
    ...
    return (
        ...
        >
            <AdminDataTable
                ...
                toolbarActiveFilters={
                    <>
                        ...
                        {supportStatus !== "all" ? (
                            <AdminFilterChip
                                label={`支持：${channelModelSupportMeta(supportStatus).label}`}
                                onRemove={() => {
                                    setSupportStatus("all");
                                    setPage(1);
                                }}
                ...
                toolbarFilters={
                    <>
                        ...
                        <Select
                            ...
                            options={[
                                { label: "全部支持状态", value: "all" },
                                { label: "可用", value: "ready" },
                                { label: "计划支持", value: "planned" },
            ...
            <Drawer
                ...
                footer={
                    isChannelModelReadOnly(editing) ? (
            ...
            >
                <Form className="admin-model-editor-form" form={form} layout="vertical" requiredMark={false} disabled={isChannelModelReadOnly(editing)} onValuesChange={handleFormValuesChange}>
                    ...
                    {isChannelModelReadOnly(editing) ? (
                        <div className="mb-4 rounded-md border border-border bg-muted/20 p-3 text-sm text-foreground/70">{editing?.supportReason || "该模型尚未完成项目执行器适配与验证，本阶段仅展示官方目录信息。"}</div>
                    ) : null}
