...
describe("xAI frame role validation", () => {
    test("rejects an end frame before any paid request is submitted", async () => {
        let postCalls = 0;
        const deps = {
            transport: {
        ...
        await expect(
            createOpenAIVideoTask(deps, config as never, "grok-imagine-video-1.5", "保持动作连续", [{ id: "last", name: "last.png", type: "image/png", dataUrl: "data:image/png;base64,AAAA", url: "" }], {
                videoEditOperation: "image_to_video",
                videoEndFrameNodeId: "last",
            }),
        ).rejects.toThrow("不支持尾帧");
