...
test("admin CSS stays scoped and its regression test remains inside panic guard", async () => {
    const [cssSource, packageSource] = await Promise.all([Bun.file(new URL("../src/styles/admin-ui.css", import.meta.url)).text(), Bun.file(new URL("../package.json", import.meta.url)).json()]);
    const drawerLightOverrides = sourceSection(cssSource, ":root:not(.dark) .admin-model-editor-drawer .ant-drawer-header", "@media (max-width: 899px)");

    expect(cssSource).not.toMatch(/(?:^|\n)\.ant-/);
    expect(cssSource).not.toContain(".dark .ant-");
    expect(drawerLightOverrides).not.toMatch(/#[0-9a-f]{3,8}\b|\brgba?\(/i);
    expect(packageSource.scripts).not.toHaveProperty("pretest");
    expect(packageSource.scripts["test:admin-ui"]).toBe("bun test test/admin-ui-regressions.test.ts");
    expect(packageSource.scripts.test).toContain("run-with-panic-guard.mjs -- bun run test:admin-ui");
});
