...
官方F6确认框交互尚未由浏览器复验，本轮不操作用户Edge或改用Chrome。其余6项网络失败已增加仅测试模式启用的精确fixture，并通过非浏览器测试，但也不宣称整个E2E已重跑通过。阶段6状态必须记为自动化源码通过、人工E2E门禁未闭合；阶段7不自动放行。需用户决定先手工验证候选，或明确将F6与视觉项一起留到阶段8验收。
