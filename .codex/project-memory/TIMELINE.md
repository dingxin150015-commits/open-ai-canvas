...
## 2026-09-09：v1.2.7阶段9稳定晋升

用户完成当前生产环境可安全观察项的Microsoft Edge复验并反馈暂未发现问题。功能检查点37bb2375经dry-run后非强制快进到用户Fork候选与stable；`origin/main`保持11931d00，默认分支仍为stable，未创建PR/Release或触发远程CI。F6无法在生产环境安全构造保存失败，转为隔离测试环境门禁且不记为通过。运行Web 0d815d15、Backend ae01c5d6保持healthy/restart0；官方main的新尾差83d9622未纳入。

## 2026-09-09：Edge首轮失败项修复并部署

用户Edge确认版本/导航/基本画布布局后，报告17371重复请求、报价400、暗色点网格不可辨和导演台入口未找到。601e5ada取消普通页面Runtime自动探测并按精确价格档前置拦截无效报价；0d815d15增强暗色点并增加宽/窄导演台入口。专项34、完整Web1624、类型/格式/构建通过；只替换Web为0d815d15，Backend与真实卷不变，等待Edge复验。
...
## 2026-09-08：阶段6自动化源码门禁

源码88087501修正发布标签/构建身份并完善全测试入口。Backend全量、Web1624、Agent Linux322+Windows6、构建与静态配置检查通过。官方CI归档可读取，明确6项网络场景断言和F6确认框失败；精确fixture已有单测，F6未浏览器复验。停止于人工门禁，不push/部署。详见阶段6记录。

## 2026-09-08：v1.2.7阶段5选择性适配

原e027aa4/f5fadb7/bcb6c6d分别部分适配为8082e9e2/2aebd53e/d10c5e32；不恢复失败UI2的撑满/滚动Dock。加入真实画布宽度布局、主题隔离、权威报价能力和共享自动发现，同时保留显式重连替代旧请求。类型/1278默认测试/23独立专项/构建通过；未push、部署、真实连接或Edge验收。详情见阶段5记录。

## 2026-09-08：v1.2.7官方集成检查点

按用户阶段3–4授权从5c1203e建立独立候选，合并371bc68。63冲突路径/184区块处理后，Backend隔离CGO全量与Web干净类型、1255测试、构建通过；本提交保留双父历史。UI补丁重放/push/部署/Edge仍未执行。详见 `UPSTREAM_V127_STAGE34_20260908.md`。
...
## 2026-09-04：批准固定 `v1.2.5` 双轨维护与阶段 0–4

- 用户接受“双轨来源、单树交付”：官方稳定 Release 提供上游功能和修复，本地自研功能由 fork 的独立模块/补丁维护，最终只交付一棵验证后的集成代码树。
- 本轮固定 `v1.2.5@f8e87bcc4ce3e6f7eae7a89dc8b9231801116072`；`official/main@7ec5173` 的 6 个新提交保持在外。
- 用户批准阶段 0–4 连续执行并在冲突解决后停止；阶段 5 全量验证、阶段 6 merge commit、阶段 7 候选 push、阶段 8 稳定晋升、阶段 9 部署仍需新批准。
- Astravia Windows 被确定为阶段 0–9 全部完成后的辅助工作台和第二审阅面；当前不安装、不启动、不接触生产数据或密钥。

## 2026-09-04：`v1.2.5` 阶段 0–2

- 阶段 0 将双轨维护与阶段 0–9 门禁固化为 `44e8c10`；当前历史检查点经 dry-run 后从 `e02cee1` fast-forward 推送到 `44e8c10`，远程 SHA 复核一致。
- 从同一提交建立并推送 `origin/codex/dingxin-stable@44e8c10`，未 force、未改 `origin/main@11931d0`、未创建 PR 或触发部署。
- fetch 官方 `v1.2.5` 到隔离审计 ref；标签对象 `2a45d0c`、peeled commit `f8e87bc` 与远端一致，但标签无可验证 Git signature。
