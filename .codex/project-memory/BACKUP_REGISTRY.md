...
## BKP-20260827-150444-STAGE15-PREDEPLOY

| 字段                    | 值                                                                 |
| ----------------------- | ------------------------------------------------------------------ |
| 状态                    | `verified`, `protected`                                            |
| 私有位置                | `<private-backup-root>\stage15-predeploy-20260827-150444`          |
| 数据库/WAL/SHM          | 2,650,112 / 4,152,992 / 32,768 bytes                               |
| 数据库 SHA-256          | `A0FB0AD53CC415FFE9001062A8F9D904ECF7035D7A355E9B52BC64A85569ADF0` |
| WAL SHA-256             | `34E39F49D8F80856565D60FC5575E318CCC32167246DE00337B36F508BB25B32` |
| SHM SHA-256             | `8E86FE082E4DC94264EA2587E2D8F0900C39D4F4DF5F80CB2DC5447D0C4FF44F` |
| `.settings-key` SHA-256 | `E4B4B107C3060A18D65AD3AAD4E329D063A924A71CCF23EAB15B9ABF8120A6D5` |
| 完整性 / 外键 / 表      | `ok` / 0 / 60                                                      |
| 业务计数                | 用户 1、会话 1、渠道 1、模型 320、任务 1、资源 1、素材 1           |
...
## BKP-20260901-114427-ROUTE-B-PREDEPLOY

| 字段                    | 值                                                                     |
| ----------------------- | ---------------------------------------------------------------------- |
| 状态                    | `verified`, `protected`, `pre-route-b`                                 |
| 私有位置                | `<private-backup-root>\route-b-predeploy-20260901-114427`              |
| 源码                    | fork检查点 `e02cee1`；部署标识提交 `c3f5479`                           |
| 原始数据库 SHA-256      | `8A9FEFB38E0478B3947F93E58DFAA8EB5967239D08CD108C099E3EF82950157B`     |
| 原始 WAL SHA-256        | `640C18769A0F39CABF4E9E844529F235386E9BFA1DBA2CCE85543885E63CD3E8`     |
| 原始 SHM SHA-256        | `773E8F51D9595C9C68906ED1E0C63A538376C79D085AF2CC0258C0580B59B5A6`     |
| `.settings-key` SHA-256 | `E4B4B107C3060A18D65AD3AAD4E329D063A924A71CCF23EAB15B9ABF8120A6D5`     |
| 迁移标记 SHA-256        | `0435B398CEC770046D79B55E9C2E6AB06D236A7759FBB44503266E1DFEA0C8F3`     |
| 升级前完整性/外键/表    | `ok` / 0 / 60                                                          |
| 升级前业务计数          | 用户1、渠道1、模型320、任务3、资源5、素材5、画布1                      |
| 克隆迁移                | 两次healthy，错误0；72表，skills/versions/files 33/33/33，业务计数稳定 |
| 升级后真实卷            | `ok` / 外键0 / 72表；关键Schema和业务计数与演练一致                    |

逐文件ACL仅当前用户、SYSTEM、Administrators；Git bundle验证完整。升级前回滚镜像：Backend `open-ai-canvas-backend:pre-route-b-20260901-114427`（`f94f555b...`），Web `open-ai-canvas-web:pre-route-b-20260901-114427`（`31417063...`）。

路线B运行候选：Backend `8182cc0a...`、Web `522109d7...`，真实卷保持 `open-ai-canvas_backend-data`。升级后容器healthy/restart0/OOMfalse，版本 `v1.1.4+local.4ba.e02cee1`。当前未完成Edge人工验收；若仅代码/UI失败，优先回滚镜像并保留已迁移数据库。只有确认数据迁移错误且用户再次批准时，才恢复原始DB/WAL/SHM、匹配密钥和迁移标记；禁止删除卷。

## BKP-20260905-143709-STAGE5-V125

| 字段 | 值 |
| --- | --- |
| 状态 | `verified`, `protected`, `pre-v1.2.5-merge` |
| 私有位置 | `<private-backup-root>\stage5-v125-premerge-20260905-143709` |
| 预迁移 DB/WAL/SHM | 3,207,168 / 4,247,752 / 32,768 bytes |
| 预迁移 DB/WAL SHA-256 | `E5414A370C1C1FDDE697A097BDA695869DC2007CC54D55F4F17231966C288CFB` / `77D6239224CFFEFBBA08EBCE3C95F8C58ADC32A96F63CF8A836074C6AF387921` |
| `.settings-key` / 迁移标记 SHA-256 | `E4B4B107C3060A18D65AD3AAD4E329D063A924A71CCF23EAB15B9ABF8120A6D5` / `0435B398CEC770046D79B55E9C2E6AB06D236A7759FBB44503266E1DFEA0C8F3` |
| 预迁移完整性 / 外键 / 表 | `ok` / 0 / 72 |
| 克隆迁移 | 同一独立卷连续两次无网络启动均 `healthy`；迁移后 `ok` / 外键0 / 80表 |
| 迁移后 DB SHA-256 | `57CB5FA8CD270824DC8CA6089A53B7E6221125BA8B217B4AE3C04091AE41786B` |
| 业务计数 | 用户1、会话1、渠道1、模型320、任务3、资源5、素材5、画布2；迁移前后保持 |
| 候选镜像 | Backend `sha256:6b04464b...`；Web `sha256:36dc2302...` |
