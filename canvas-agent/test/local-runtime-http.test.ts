...
test("Local Runtime keeps the signed JSON body cap at 30MB", async () => {
  const source = await fs.readFile(
    new URL("../src/local-runtime.ts", import.meta.url),
    "utf8",
  );
  assert.match(source, /limit:\s*"30mb"/);
  assert.doesNotMatch(source, /limit:\s*"64mb"/);
});
...
test("route-scoped CORS keeps every method registered on the same path", async () => {
  await withRuntime(async ({ server }) => {
    const preflight = await request(server, {
      ...
      headers: {
        ...
        "Access-Control-Request-Headers":
          "content-type,x-framefield-runtime-session,x-framefield-runtime-timestamp,x-framefield-runtime-nonce,x-framefield-runtime-proof",
      },
    });
    assert.equal(preflight.status, 204);
    ...
    assert.equal(
      ...
      "x-framefield-runtime-session,x-framefield-runtime-timestamp,x-framefield-runtime-nonce,x-framefield-runtime-proof,content-type",
    );
  });
});
  ...
  await withRuntime(async ({ server }) => {
    ...
    for (const method of ["GET", "POST"]) {
      const removed = await request(server, {
        method,
        path: `/runtime/pair/${challenge.challengeId}`,
...
test("Dreamina effect routes require a signed dreamina:generate request before invocation", async () => {
  ...
  await withRuntime(
    async ({ server }) => {
      ...
      const substitutedBody = Buffer.from(
        JSON.stringify({ consumerId: "substituted" }),
      );
      const substituted = await request(server, {
        method: "POST",
        path,
...
test("production Dreamina effect claim composes signing scope and strict JSON in the real runtime app", async () => {
  await withProductionDreaminaRuntime(async ({ server }) => {
    ...
    assert.equal(
      (JSON.parse(unexpected.body) as { code?: string }).code,
    ...
    assert.equal(accepted.status, 200);
    assert.equal(
      (JSON.parse(accepted.body) as { result?: { status?: string } }).result
        ?.status,
      "claimed",
    );
  });

  await withProductionDreaminaRuntime(
    async ({ server }) => {
      const key = browserKey();
      const challenge = await challengeRequest(server, key.publicJwk);
      const session = await exchangeRequest(server, key.privateKey, challenge);
...
test("signed session and registration revoke invalidate the intended authorization", async () => {
  await withRuntime(async ({ server, manager }) => {
    const key = browserKey();
    const challenge = await challengeRequest(server, key.publicJwk);
    const firstSession = await exchangeRequest(
      server,
      key.privateKey,
      challenge,
    );
    ...
    const secondChallenge = await challengeRequest(
      server,
      undefined,
      firstSession.keyId,
    );
    const secondSession = await exchangeRequest(
      server,
      key.privateKey,
      secondChallenge,
    );
    const thirdChallenge = await challengeRequest(
      server,
      undefined,
      firstSession.keyId,
    );
    const thirdSession = await exchangeRequest(
      server,
      key.privateKey,
      thirdChallenge,
    );
    ...
    assert.equal(
      JSON.parse(rejectedRegistration.body).code,
...
async function withProductionDreaminaRuntime(
  run: (fixture: {
    server: Server;
    manager: LocalRuntimeSessionManager;
  }) => Promise<void>,
  scopes?: ConstructorParameters<
    typeof LocalRuntimeSessionManager
  >[0]["scopes"],
) {
  const configDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "dreamina-production-http-"),
  );
  const idempotencyKey = "dreamina-http-effect-task-0001";
  await fs.writeFile(
    path.join(configDir, "dreamina-runtime-state.json"),
    JSON.stringify({
      version: 1,
      records: [
        {
...
function signedHeaders(
  ...
  return {
    ...
    "X-Framefield-Runtime-Proof": crypto
      .sign("sha256", Buffer.from(canonicalRuntimeJson(payload)), {
        key: privateKey,
