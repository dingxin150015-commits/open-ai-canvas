...
export function createLocalRuntimeApp(
  options: CreateLocalRuntimeAppOptions,
): Express {
  ...
  app.get("/health", (_req, res) => {
    const health = modules.reduce<Record<string, string | number | boolean>>(
      (result, module) => {
  ...
  app.post(
    ...
    route((req, res) => {
      ...
      if ("publicKeyJwk" in body) {
        ...
        res.json(
          options.sessionManager.createChallenge(requiredOrigin(req), {
  ...
  app.post(
    ...
    route((req, res) => {
      ...
      res.json(
        options.sessionManager.exchange(requiredOrigin(req), {
  ...
  for (const module of modules) {
    for (const item of module.routes) {
      ...
      const handlers = [guard, item.handler];
  ...
  app.use((_req, res) =>
    res
      .status(404)
      .json({ ok: false, code: "not_found", message: "未找到本机运行时路由" }),
  );
...
function validateModules(modules: readonly LocalRuntimeModule[]) {
  ...
  for (const module of modules) {
    if (
      module.descriptor.id !== "canvas-agent" &&
      module.descriptor.id !== "dreamina" &&
      module.descriptor.id !== "portrait-clearance"
    ) {
      throw new Error(
        `Unsupported Local Runtime module id: ${module.descriptor.id}`,
      );
    }
    if (ids.has(module.descriptor.id))
      throw new Error(
        `Duplicate Local Runtime module id: ${module.descriptor.id}`,
      );
    ids.add(module.descriptor.id);
    for (const item of module.routes) {
      const key = `${item.method} ${item.path}`;
      if (routes.has(key))
        throw new Error(`Duplicate Local Runtime route: ${key}`);
      if (
        ...
        item.path
          ...
          .some(
            (segment) =>
              segment.includes(":") && !/^:[A-Za-z][A-Za-z0-9]*$/.test(segment),
          )
      ) {
...
function corsPolicies(
  ...
  const policies = new Map<string, RuntimeCorsPolicy>([
    ...
    [
      "/runtime/session/challenge",
      { methods: ["POST"], headers: ["content-type"], trustedOrigin: true },
    ],
    [
      "/runtime/session/exchange",
      { methods: ["POST"], headers: ["content-type"], trustedOrigin: true },
    ...
    [
      ...
      {
        methods: ["GET"],
        headers: protectedCorsHeaders("GET"),
        trustedOrigin: true,
      },
    ],
    [
      ...
      {
        methods: ["POST"],
        headers: protectedCorsHeaders("POST"),
        trustedOrigin: true,
      },
    ],
    [
    ...
    for (const item of module.routes) {
      ...
      policies.set(item.path, {
        ...
        headers: uniqueStrings([
          ...(previous?.headers || []),
          ...protectedCorsHeaders(item.method, item.lastEventId),
        ]),
        ...(previous?.publicInfo ? { publicInfo: true } : {}),
        ...(previous?.trustedOrigin || item.legacy || !previous
          ? { trustedOrigin: true }
          : {}),
        ...(previous?.legacyOrigins || item.legacy
          ? {
              legacyOrigins: previous?.legacyOrigins || legacyOrigins,
              legacyHeaders: uniqueStrings([
                ...(previous?.legacyHeaders || []),
                ...legacyHeaders,
              ]),
            }
          : {}),
...
function requiredRuntimeSessionId(res: Response) {
  const value = (
    res.locals.runtimeSession as { sessionId?: unknown } | undefined
  )?.sessionId;
