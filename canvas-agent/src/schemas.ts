...
const agentCandidateCategorySchema = z.enum([
  "environment",
  "prop",
  "material",
  "other",
...
const projectCandidateSchema = z.object({
  ...
  name: z.string().min(1),
  category: agentCandidateCategorySchema,
...
export const canvasOpSchema = z.discriminatedUnion("type", [
  ...
  z
    ...
    .passthrough(),
  z
    .object({ type: z.literal("set_viewport"), viewport: viewportSchema })
    .passthrough(),
  z
    .object({ type: z.literal("select_nodes"), ids: z.array(z.string()) })
    .passthrough(),
  z
    .object({
...
export const toolInputSchemas = {
  ...
  canvas_get_generation_tasks: z.object({
    status: z.string().optional(),
    nodeIds: z.array(z.string()).optional(),
    limit: z.number().int().min(1).max(200).optional(),
  }),
  canvas_get_resources: z.object({
    nodeIds: z.array(z.string()).optional(),
    status: z.string().optional(),
  ...
  project_list_units: z.object({
    projectId: projectIdSchema,
    kind: z.string().optional(),
    status: z.string().optional(),
  }),
  project_extract_asset_candidates: z.object({
    projectId: projectIdSchema,
    candidates: z.array(projectCandidateSchema).min(1).max(100),
  }),
  project_confirm_asset_candidate: z.object({
    projectId: projectIdSchema,
    candidateId: z.string().min(1),
    assetId: z.string().optional(),
  }),
  project_create_or_update_shots: z.object({
    projectId: projectIdSchema,
    shots: z.array(projectShotSchema).min(1).max(100),
  }),
  project_link_shot_asset: z.object({
    projectId: projectIdSchema,
  ...
  project_start_workflow_step: z.object({
    projectId: projectIdSchema,
    stepId: z.string().min(1),
  }),
  project_link_asset: z.object({
    projectId: projectIdSchema,
    assetId: z.string().min(1),
    category: assetCategorySchema,
  }),
  project_upsert_asset_version: z.object({
    projectId: projectIdSchema,
    assetId: z.string().min(1),
    prompt: z.string().optional(),
  ...
  project_register_task_output: z.object({
    projectId: projectIdSchema,
    stepId: z.string().min(1),
...
export const toolDescriptions: Record<ToolName, string> = {
  ...
  canvas_get_context:
    "读取面向 Agent 的语义化画布上下文：节点用途、连接关系、选区、资源引用、生成状态和状态哈希。优先于直接猜测节点 metadata。",
  canvas_find_nodes:
    "按标题、内容、提示词、类型、状态、资产或工作流检索真实节点，返回可用于后续写操作的节点 id。",
  canvas_get_node:
    "按真实节点 id 精确读取单个节点的语义、完整安全 metadata、资源状态和关联连线；找不到时明确返回未找到。",
  canvas_get_connection:
    "按真实连线 id 精确读取连线端点、端点节点摘要和 handle 信息；找不到时明确返回未找到。",
  canvas_get_generation_tasks:
    "读取当前画布节点已经绑定的生成任务观察状态，包括 taskId、节点、任务状态、进度、阶段和错误；这是画布快照观察，不会主动轮询上游。",
  canvas_get_resources:
    "读取当前画布引用的媒体资源清单，包括资源/资产引用、类型、尺寸、大小、时长和就绪状态；不会返回媒体 URL。",
  canvas_validate_ops:
    "在真正写入前校验批量操作中的节点 id、连接关系和参数，避免对不存在节点或错误资源执行操作。",
  ...
  canvas_create_workflow:
    "创建语义化工作流/流水线。character_cards 是角色拆分图片卡片，character_three_view 是角色三视图，storyboard_video 是分镜剧情视频。媒体节点必须有 prompt/content；已有素材使用先读取到的真实 node id 填入 referenceNodeIds。自动分配 id、根据真实节点尺寸布局、建立 edges/referenceRefs/referenceNodeIds 连线并返回可验证结果；不要用批量文本节点代替工作流。",
  canvas_create_node:
    "创建任意类型节点：text、script、image、video、audio、frame。适合创建脚本、媒体占位、背板或自定义 metadata 节点。",
  ...
  canvas_create_generation_flow:
    "创建通用生成流程：提示词文本节点、对应类型的生成目标节点和参考节点连线，可用于文案、生图、视频或音频。",
  ...
  canvas_run_generation:
    "触发指定节点生成；对已有生成任务明确重试时传 retry=true，以复用产品级重试幂等身份。",
  project_get_context:
    "读取当前短剧项目的章节、画布、资产、镜头、候选和工作流事实。",
  project_list_units: "按类型或状态筛选当前短剧项目的章节/项目单元。",
  project_extract_asset_candidates:
    "将分镜识别出的场景、道具、素材或其他需求登记为待确认资产候选；角色只在剧情章节提取，不通过此工具重复创建。",
  project_confirm_asset_candidate:
    "确认一个资产候选，创建正式资产或关联已有个人资产。",
  project_create_or_update_shots:
    "创建或更新项目镜头业务数据，不把镜头状态写进画布 metadata。",
  ...
  project_upsert_asset_version:
    "为项目资产创建新的设定和提示词版本，保留历史版本。",
  project_register_task_output:
    "将成功生成任务挂到流程步骤，并登记到具体资产版本和资源表示。",
};
