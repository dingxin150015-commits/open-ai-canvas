...
import {
  assertCodexThreadWorkspace,
  codexThreadInWorkspace,
  resolveCodexThread,
...
export async function listCodexThreads(
  ...
  const data = Array.isArray(field(result, "data"))
    ? (field(result, "data") as unknown[])
        ...
        .filter((thread) => codexThreadInWorkspace(thread, options.cwd))
    : [];
  return {
    data,
...
export async function verifyCodexThreadWorkspace(
  emit: AgentEmit,
  threadId: string,
  cwd: string,
) {
...
class CodexAppClient {
  ...
  private handle(message: Json) {
    ...
    if (message.error && this.pending.has(id))
      return this.reject(
        id,
        String(field(message.error, "message") || "Codex request failed"),
      );
  ...
  private handleNotification(method: string, params: Json) {
    ...
    if (event.type === "turn.completed") {
      const turnId = String(
        field(params, "turnId") || field(field(params, "turn"), "id") || "",
      );
      ...
      if (pending) {
        ...
        error
          ? pending.reject(
              new Error(String(field(error, "message") || "Codex turn failed")),
            )
...
export function canvasAgentMcpCommand() {
  ...
  const entry = path.resolve(
    current || fileURLToPath(new URL("./index.js", import.meta.url)),
  );
  const tsx = path.join(
    path.dirname(entry),
    "..",
    ...
    mcp_servers: {
      yingce: {
        ...
        tool_timeout_sec: Math.ceil(
          (CANVAS_GENERATION_CONTINUATION_TIMEOUT_MS +
            INTERNAL_CANVAS_MCP_TIMEOUT_MARGIN_MS) /
            1_000,
        ),
      },
    },
...
function normalizeCodexNotification(
  ...
  if (method === "thread/started")
    return {
      type: "thread.started",
      thread_id: field(field(params, "thread"), "id"),
    };
  ...
  if (method === "item/completed")
    return {
      type: "item.completed",
      item: normalizeItem(field(params, "item")),
    };
  if (method === "error")
    return { type: "error", message: field(params, "message") };
  return null;
}
  ...
  try {
    for (const skill of skills.slice(0, 8)) {
      ...
      const temporaryRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "infinite-canvas-skill-"),
      );
      ...
      if (packageFiles.length) {
        ...
        await fs.writeFile(
          ...
          skillMarkdown(name, skill.name, skill.description, instruction),
          "utf8",
        );
      }
...
function skillMarkdown(
  ...
  const summary = String(description || title)
    .trim()
    .slice(0, 500)
    .replace(/[\r\n]+/g, " ");
  return [
...
function hasSkillFrontmatter(value: string) {
  ...
  return (
    /^name\s*:/m.test(frontmatter) && /^description\s*:/m.test(frontmatter)
  );
}
