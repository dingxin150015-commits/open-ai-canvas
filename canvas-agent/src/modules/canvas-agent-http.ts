...
export function createCanvasAgentHttpModule(
  ...
  const routes: LocalRuntimeProtectedRoute[] = [
    ...
    canvasRoute(
      ...
      "/canvas/state",
      (req, res) => {
        const result = session.updateState(
          jsonBody(req),
          queryValue(req, "clientId") || undefined,
        );
        if (!result) {
          res.json({ ok: true });
          return;
    ...
    canvasRoute("POST", "/api/tools", async (req, res) => {
      ...
      res.json({
        ok: true,
        result: await session.callTool(body.name, body.input || {}),
      });
    }),
    canvasRoute(
      "GET",
      ...
      (req, res) => {
        const workspace = ensureCanvasWorkspace(
          config,
          queryValue(req, "canvasId"),
        );
    ...
    canvasRoute(
      ...
      async (req, res) => {
        ...
        updateCanvasWorkspace(config, workspace.canvasId, {
    ...
    canvasRoute("POST", "/agent/codex/turn", (req, res) => {
      ...
      void (async () => {
        ...
        if (!res.headersSent)
          res.status(500).json({ ok: false, error: publicCanvasError(error) });
      });
    }),
  ...
  return {
    ...
    routes,
    onRuntimeSessionRevoked: (sessionId) =>
      session.closeRuntimeSession(sessionId),
    publicHealth: () => {
  ...
  return value.slice(0, 8).flatMap((item) => {
    ...
    const instruction =
      typeof input.instruction === "string"
        ? input.instruction.trim().slice(0, 24_000)
        : "";
    ...
    return [
      {
        ...(typeof input.skillId === "string"
          ? { skillId: input.skillId.trim().slice(0, 120) }
          : {}),
        name,
...
function parseAgentSkillFiles(value: unknown) {
  ...
  for (const item of value.slice(0, 512)) {
    ...
    const input = item as Record<string, unknown>;
    const filePath =
      typeof input.path === "string" ? normalizeAgentSkillPath(input.path) : "";
    const contentBase64 =
      typeof input.contentBase64 === "string" ? input.contentBase64.trim() : "";
    if (!filePath || paths.has(filePath) || !validBase64(contentBase64))
      return [];
    ...
    files.push({
      ...
      ...(typeof input.mimeType === "string"
        ? { mimeType: input.mimeType.trim().slice(0, 255) }
        : {}),
      contentBase64,
    });
  }
