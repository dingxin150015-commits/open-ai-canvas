...
export function createDefaultLocalRuntimeModules(
  config: LocalRuntimeConfig,
): LocalRuntimeModule[] {
  return [
    ...
    createDreaminaHttpModule({
      ...
      referenceRoots: () => [
        ...
        ...Object.values(config.canvases ?? {}).map(
          (canvas) => canvas.workspacePath,
        ),
      ],
    }),
    createPortraitClearanceHttpModule({
      ownerId: ensureRuntimeOwnerId(config),
      configDir: CONFIG_DIR,
    }),
  ];
...
export function startLocalRuntime(
  ...
  const endpoint =
    requestedPort === 0 ? config.url : `http://127.0.0.1:${requestedPort}`;
  const authority =
    requestedPort === 0 ? "127.0.0.1:0" : `127.0.0.1:${requestedPort}`;
  const modules = [
    ...(options.modules ?? createDefaultLocalRuntimeModules(config)),
  ];
  const scopes = [
    ...new Set([
      ...LOCAL_RUNTIME_DEFAULT_SCOPES,
  ...
  const disposeModules = async () => {
    ...
    if (errors.length)
      throw new AggregateError(errors, "Local Runtime module disposal failed");
  };
  const ready = (async () => {
    try {
      ...
      log(
        "Codex MCP: codex mcp add yingce -- npx -y @ddcat666/open-ai-canvas-agent mcp",
      );
    } catch (startupError) {
      sessions.dispose();
  ...
  const close = () => {
    closePromise ??= (async () => {
      try {
        ...
        /* Startup error remains observable through ready. */
      }
      sessions.dispose();
      const errors: unknown[] = [];
      try {
