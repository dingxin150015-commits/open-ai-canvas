...
func (s *Service) log(userID string, taskID string, level string, message string, payload string) error {
	...
	return s.repo.Create(&model.TaskLog{ID: newID(), UserID: userID, TaskID: taskID, TraceID: traceID, RequestID: requestID, Level: level, Message: message, Payload: safeTaskLogPayload(payload)})
}
