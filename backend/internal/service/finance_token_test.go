...
func TestEnrichAPICallLogKeepsUserFacingCategoryAndRedactsUpstreamFailureDetails(t *testing.T) {
	...
	if strings.Contains(log.Error, "invalid parameter: size") || !strings.Contains(log.Error, "原始消息已隐藏") {
		t.Fatalf("EnrichAPICallLog() leaked or failed to mark hidden upstream detail: %q", log.Error)
	}
