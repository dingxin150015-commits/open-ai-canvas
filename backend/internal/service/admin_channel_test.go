...
func TestImportAdminChannelModelsOnlyImportsSelectedModels(t *testing.T) {
	...
	if imported[0].Enabled || imported[0].PriceConfigured || imported[0].SupportStatus == "" || imported[0].CatalogSource == "" {
		t.Fatalf("import lost catalog safety metadata: status=%q source=%q", imported[0].SupportStatus, imported[0].CatalogSource)
	}
}
