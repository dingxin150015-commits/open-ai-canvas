...
func TestXAIVideoReferenceOperationIgnoresStaleStartFrameMetadata(t *testing.T) {
	body, err := xaiVideoRequestBody(canvasGenerationInput{
		...
		Metadata: map[string]interface{}{"videoEditOperation": "reference_to_video", "videoStartFrameNodeId": "character", "videoEndFrameNodeId": "stale-end"},
	})
	if err != nil {
		t.Fatalf("xaiVideoRequestBody() error = %v", err)
	}
...
func TestXAIVideoBodyRejectsEndFrameMetadata(t *testing.T) {
	_, err := xaiVideoRequestBody(canvasGenerationInput{
		Config:          providerConfig{Model: "grok-imagine-video-1.5", InterfaceType: "xai-video"},
		ReferenceImages: []providerMedia{{ID: "last", DataURL: testReferenceImageDataURL}},
		Metadata:        map[string]interface{}{"videoEditOperation": "image_to_video", "videoEndFrameNodeId": "last"},
	})
...
func TestValidateGenerationInterfaceRejectsMismatchedType(t *testing.T) {
	...
	if err := validateGenerationInterface("image", "dashscope-image"); err != nil {
		t.Fatalf("validateGenerationInterface(dashscope-image) error = %v", err)
	}
	if err := validateGenerationInterface("video", "dashscope-video"); err != nil {
		t.Fatalf("validateGenerationInterface(dashscope-video) error = %v", err)
	}
	if _, ok := declarativeProtocolAdapterForContext(context.Background(), "dashscope-video"); ok {
		t.Fatal("DashScope video must stay on the model-specific host Provider")
	}
}
