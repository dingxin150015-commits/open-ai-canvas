...
func providerMediaHydrationPolicyFor(ctx context.Context, input canvasGenerationInput) providerMediaHydrationPolicy {
	...
	case string(model.ChannelInterfaceNewAPIVideo), string(model.ChannelInterfaceNewAPIChannel1), string(model.ChannelInterfaceNewAPIChannel2), string(model.ChannelInterfaceVolcengineArkVideo), string(model.ChannelInterfaceMiniMaxVideo), "dashscope-video":
...
func isNewAPIChannel2TaskNotReady(interfaceType string, taskID string, err error) bool {
	...
	// Classify the structured response before user-facing redaction. Never expose
	// raw messages or classify an unstructured/partial match as retryable.
	nestedError, _ := payload["error"].(map[string]any)
	nestedData, _ := payload["data"].(map[string]any)
	for _, entry := range []map[string]any{payload, nestedError, nestedData} {
		for _, key := range []string{"code", "message", "msg"} {
			if value, ok := entry[key].(string); ok && strings.EqualFold(strings.TrimSpace(value), "task_not_exist") {
				return true
			}
		}
	}
...
func xaiVideoRequestBody(input canvasGenerationInput) (xaiVideoRequest, error) {
	...
	if endFrameID != "" {
		return xaiVideoRequest{}, errors.New("xAI 视频协议不支持尾帧输入")
	}
	if startFrameID == "" {
