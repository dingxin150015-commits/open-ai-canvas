...
func main() {
	...
	r.Use(gin.LoggerWithFormatter(func(param gin.LogFormatterParams) string {
		return fmt.Sprintf("%s - [%s] request_id=%s trace_id=%s \"%s %s\" %d %s %s\n", param.ClientIP, param.TimeStamp.Format(time.RFC3339), param.Request.Header.Get("X-Request-ID"), param.Request.Header.Get("X-Canvas-Trace-ID"), param.Method, redactCanvasSharePath(param.Path), param.StatusCode, param.Latency, param.ErrorMessage)
...
const corsAllowedHeaders = "Accept, Content-Type, Authorization, X-Requested-With, X-Request-ID, X-Canvas-Trace-ID, X-Canvas-Scene, X-Idempotency-Key, X-Canvas-Upstream-URL, X-Canvas-Upstream-Format, X-Canvas-Allow-Local-Channel, X-Canvas-Upstream-Base-URL, X-Canvas-Comfy-Bridge-Token, X-Canvas-Bridge-Token"
