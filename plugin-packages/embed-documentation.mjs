...
function withoutGeneratedContract(document) {
  ...
  if (end < 0) {
    throw new Error(
      "docs/interface.md contains an unterminated generated manifest contract",
    );
  }
...
const packageDirectories = entries
  ...
  .filter(
    (name) => requestedPackageIDs.size === 0 || requestedPackageIDs.has(name),
  )
  .sort();
...
console.log(
  `embedded complete documentation into ${updated} protocol packages`,
);
