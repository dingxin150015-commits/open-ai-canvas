...
const conditional = (condition, thenValue, elseValue = null) => ({
  $if: { condition, then: thenValue, else: elseValue },
});
const first = (value) => ({ $first: value });
const sorted = (value) => ({ $sortByOrder: value });
const mediaWithRoles = (path, roles) =>
  filter(sorted(ref(path)), "media", { $in: [ref("media.role"), roles] });
const firstMediaWithRoles = (path, roles) => first(mediaWithRoles(path, roles));
const firstMediaFieldWithRoles = (path, roles, field) =>
  first(map(mediaWithRoles(path, roles), "media", ref(`media.${field}`)));
...
const imageParams = [
  ...
  [
    ...
    "media[]",
    false,
    "provider image/reference fields",
    "参考图或编辑源图，role 由业务层确定。",
  ],
...
const parameters = (items) =>
  items.map(([name, type, required, mapping, description]) => ({
    name,
    type,
    required,
    mapping,
...
const jsonCreate = (path, body, extra = {}) => ({
  method: "POST",
  path,
...
const asyncResponse = (kind, overrides = {}) => ({
  ...
  [kind + "s"]: coalesce(
    ref(`response.${kind}_url`),
    ref(`response.${kind}Url`),
    ref("response.result_url"),
    ref("response.url"),
    ref(`response.data.${kind}_url`),
...
add({
  ...
  create: jsonCreate("/chat/completions", {
    ...
    messages: ref("request.messages"),
    temperature: omit(
      ref("request.providerOptions.chat-completion.temperature"),
    ),
    top_p: omit(ref("request.providerOptions.chat-completion.top_p")),
    max_tokens: omit(
      coalesce(
        ref("request.extra.max_tokens"),
        ref("request.providerOptions.chat-completion.max_tokens"),
      ),
    ),
    tools: omit(ref("request.providerOptions.chat-completion.tools")),
    tool_choice: omit(
      ref("request.providerOptions.chat-completion.tool_choice"),
    ),
    response_format: omit(
      ref("request.providerOptions.chat-completion.response_format"),
    ),
    ...
    frequency_penalty: omit(
      ref("request.providerOptions.chat-completion.frequency_penalty"),
    ),
    presence_penalty: omit(
      ref("request.providerOptions.chat-completion.presence_penalty"),
    ),
    logprobs: omit(ref("request.providerOptions.chat-completion.logprobs")),
    top_logprobs: omit(
      ref("request.providerOptions.chat-completion.top_logprobs"),
    ),
    user: omit(ref("request.providerOptions.chat-completion.user")),
  }),
  agent: jsonCreate("/chat/completions", {
    $merge: [
  ...
  agentResponse: {
    textPaths: ["choices.0.message.content", "choices.0.text"],
...
add({
  ...
  create: jsonCreate("/responses", {
    ...
    tools: omit(ref("request.providerOptions.openai-response.tools")),
    tool_choice: omit(
      ref("request.providerOptions.openai-response.tool_choice"),
    ),
    text: omit(ref("request.providerOptions.openai-response.text")),
    reasoning: omit(ref("request.providerOptions.openai-response.reasoning")),
    previous_response_id: omit(
      ref("request.providerOptions.openai-response.previous_response_id"),
    ),
    store: omit(ref("request.providerOptions.openai-response.store")),
    metadata: omit(ref("request.providerOptions.openai-response.metadata")),
    stream: omit(ref("request.providerOptions.openai-response.stream")),
    truncation: omit(ref("request.providerOptions.openai-response.truncation")),
    user: omit(ref("request.providerOptions.openai-response.user")),
  }),
  agent: jsonCreate("/responses", {
    $merge: [
  ...
  agentResponse: {
    textPaths: ["output_text"],
    reasoningPaths: ["reasoning.summary.0.text"],
    toolCallsPath: "output",
    toolCallIdPaths: ["call_id", "id"],
    toolCallNamePaths: ["name"],
    toolCallArgumentsPaths: ["arguments"],
  },
  response: {
    status: "succeeded",
    textPaths: ["output_text"],
...
add({
  ...
  auth: { type: "anthropic", field: "apiKey" },
  params: textParams,
  create: jsonCreate(
    "/v1/messages",
    {
      ...
      messages: filter(
        ...
        "message",
        ne(ref("message.role"), "system"),
      ),
      temperature: omit(ref("request.providerOptions.claude-api.temperature")),
      top_p: omit(ref("request.providerOptions.claude-api.top_p")),
      top_k: omit(ref("request.providerOptions.claude-api.top_k")),
      stop_sequences: omit(
        ref("request.providerOptions.claude-api.stop_sequences"),
      ),
      tools: omit(ref("request.providerOptions.claude-api.tools")),
      tool_choice: omit(ref("request.providerOptions.claude-api.tool_choice")),
      metadata: omit(ref("request.providerOptions.claude-api.metadata")),
      stream: omit(ref("request.providerOptions.claude-api.stream")),
      thinking: omit(ref("request.providerOptions.claude-api.thinking")),
      service_tier: omit(
  ...
  agentResponse: {
    textPaths: ["content.0.text"],
    reasoningPaths: ["content.0.thinking"],
    toolCallsPath: "content",
    toolCallIdPaths: ["id"],
    toolCallNamePaths: ["name"],
    toolCallArgumentsPaths: ["input"],
  },
  response: {
    status: "succeeded",
    text: map(
      filter(ref("response.content"), "part", eq(ref("part.type"), "text")),
      "part",
      ref("part.text"),
    ),
    reasoning: map(
      filter(ref("response.content"), "part", eq(ref("part.type"), "thinking")),
      "part",
      ref("part.thinking"),
    ),
    usage: ref("response.usage"),
    errorPaths: ["error.type"],
    messagePaths: ["error.message"],
  },
});
...
add({
  ...
  auth: { type: "google-api-key", field: "apiKey" },
  params: textParams,
  create: jsonCreate("/v1beta/models/{{model}}:generateContent", {
    ...
    systemInstruction: conditional(ref("request.instructions"), {
      parts: [{ text: ref("request.instructions") }],
    }),
  ...
  agentResponse: {
    textPaths: ["candidates.0.content.parts.0.text"],
    toolCallsPath: "candidates.0.content.parts",
    toolCallIdPaths: ["functionCall.id"],
    toolCallNamePaths: ["functionCall.name"],
    toolCallArgumentsPaths: ["functionCall.args"],
    toolCallThoughtSignaturePaths: ["thoughtSignature", "thought_signature"],
  },
  response: {
    status: "succeeded",
    text: map(
    ...
    usage: ref("response.usageMetadata"),
    errorPaths: ["error.code"],
    messagePaths: ["error.message"],
  },
});
...
add({
  ...
  create: jsonCreate("/v2/chat", {
    ...
    messages: ref("request.messages"),
    temperature: omit(
      ref("request.providerOptions.cohere-chat-v2.temperature"),
    ),
    max_tokens: omit(ref("request.extra.max_tokens")),
    stop_sequences: omit(
      ref("request.providerOptions.cohere-chat-v2.stop_sequences"),
    ),
    tools: omit(ref("request.providerOptions.cohere-chat-v2.tools")),
    tool_choice: omit(
      ref("request.providerOptions.cohere-chat-v2.tool_choice"),
    ),
    response_format: omit(
      ref("request.providerOptions.cohere-chat-v2.response_format"),
    ),
    documents: omit(ref("request.providerOptions.cohere-chat-v2.documents")),
    citation_options: omit(
      ref("request.providerOptions.cohere-chat-v2.citation_options"),
    ),
    safety_mode: omit(
      ref("request.providerOptions.cohere-chat-v2.safety_mode"),
    ),
    seed: omit(ref("request.providerOptions.cohere-chat-v2.seed")),
    stream: omit(ref("request.providerOptions.cohere-chat-v2.stream")),
  }),
  response: {
    status: "succeeded",
    textPaths: ["message.content.0.text", "text"],
    usage: ref("response.usage"),
    errorPaths: ["error.type"],
...
add({
  ...
  auth: { type: "none", field: "apiKey" },
  params: textParams,
  create: jsonCreate("/api/chat", {
    model: ref("request.model"),
    messages: ref("request.messages"),
    stream: false,
    format: omit(ref("request.providerOptions.ollama-chat.format")),
    options: omit(ref("request.providerOptions.ollama-chat.options")),
    keep_alive: omit(ref("request.providerOptions.ollama-chat.keep_alive")),
    tools: omit(ref("request.providerOptions.ollama-chat.tools")),
    think: omit(ref("request.providerOptions.ollama-chat.think")),
  }),
  response: {
    status: "succeeded",
    textPaths: ["message.content", "response"],
    reasoningPaths: ["message.thinking", "thinking"],
    errorPaths: ["error"],
    messagePaths: ["error"],
  },
});
...
for (const [id, name, vendor, baseUrl] of [
  ...
  add({
    ...
    create: jsonCreate("/v1/chat/completions", {
      model: ref("request.model"),
      messages: ref("request.messages"),
      temperature: omit(ref(`request.providerOptions.${id}.temperature`)),
      top_p: omit(ref(`request.providerOptions.${id}.top_p`)),
      max_tokens: omit(
        coalesce(
          ref("request.extra.max_tokens"),
          ref(`request.providerOptions.${id}.max_tokens`),
        ),
      ),
      tools: omit(ref(`request.providerOptions.${id}.tools`)),
      tool_choice: omit(ref(`request.providerOptions.${id}.tool_choice`)),
      response_format: omit(
        ref(`request.providerOptions.${id}.response_format`),
      ),
      stream: omit(ref(`request.providerOptions.${id}.stream`)),
      provider: omit(ref(`request.providerOptions.${id}.provider`)),
      transforms: omit(ref(`request.providerOptions.${id}.transforms`)),
      extra_body: omit(ref(`request.providerOptions.${id}.extra_body`)),
    }),
    agent: jsonCreate("/v1/chat/completions", {
      $merge: [
        ref("request.extra.agent.chatCompletion"),
        { model: ref("request.model") },
      ],
...
add({
  ...
  create: jsonCreate("/v1/images/generations", {
    ...
    image: conditional(gt(len(ref("request.images")), 0), {
      url: firstMediaFieldWithRoles(
        "request.images",
    ...
    n: conditional(
      gt(ref("request.imageCount"), 0),
      ref("request.imageCount"),
      1,
    ),
    response_format: coalesce(
      ref("request.providerOptions.grok-image.response_format"),
      "url",
    ),
    aspect_ratio: omit(ref("request.aspectRatio")),
    resolution: omit(
      coalesce(ref("request.resolution"), ref("request.quality")),
    ),
  ...
  response: {
    ...
    images: map(ref("response.data"), "item", {
      ...
      dataUrl: conditional(ref("item.b64_json"), {
        $concat: ["data:image/png;base64,", ref("item.b64_json")],
      }),
    }),
    errorPaths: ["error.code"],
    messagePaths: ["error.message"],
...
add({
  ...
  create: jsonCreate("/api/v3/images/generations", {
    ...
    size: omit({
      $switch: {
        cases: arkSeedreamRatioSizes.map(([ratio, size]) => ({
          when: eq(ref("request.aspectRatio"), ratio),
          then: size,
        })),
    ...
    sequential_image_generation: omit(
      ref(
        "request.providerOptions.volcengine-ark-image.sequential_image_generation",
      ),
    ),
    sequential_image_generation_options: omit(
      ref(
        "request.providerOptions.volcengine-ark-image.sequential_image_generation_options",
      ),
    ),
...
add({
  ...
  create: jsonCreate("/v1beta/models/{{model}}:generateContent", {
          ...
          $concatArrays: [
            ...
            map(
              ...
              conditional(
                ...
                {
                  inlineData: {
                    mimeType: { $dataMime: ref("media.dataUrl") },
                    data: { $dataPayload: ref("media.dataUrl") },
                  },
                },
                {
                  fileData: {
                    mimeType: omit(ref("media.mimeType")),
                    fileUri: ref("media.url"),
                  },
                },
              ),
            ),
    ...
    generationConfig: {
      responseModalities: coalesce(
        ref("request.providerOptions.gemini-image.responseModalities"),
        ["TEXT", "IMAGE"],
      ),
      imageConfig: {
        aspectRatio: omit(ref("request.aspectRatio")),
        imageSize: omit(
          coalesce(ref("request.resolution"), ref("request.quality")),
        ),
      },
      candidateCount: omit(ref("request.imageCount")),
      temperature: omit(
        ref("request.providerOptions.gemini-image.temperature"),
      ),
      topP: omit(ref("request.providerOptions.gemini-image.topP")),
      topK: omit(ref("request.providerOptions.gemini-image.topK")),
      seed: omit(ref("request.providerOptions.gemini-image.seed")),
    },
    safetySettings: omit(
      ref("request.providerOptions.gemini-image.safetySettings"),
    ),
    systemInstruction: omit(
  ...
  response: {
    status: "succeeded",
    images: map(
      filter(ref("response.candidates.0.content.parts"), "part", {
        $or: [ref("part.inlineData"), ref("part.inline_data")],
      }),
      "part",
      {
        dataUrl: {
          $concat: [
            ...
            ";base64,",
            coalesce(ref("part.inlineData.data"), ref("part.inline_data.data")),
          ],
        },
      },
    ),
...
add({
  ...
  configuration: config([
    { name: "secretKey", type: "secret", label: "Secret Key", required: true },
  ]),
  create: jsonCreate(
    "/",
    {
      req_key: ref("request.model"),
      prompt: ref("request.prompt"),
      image_urls: omit(map(ref("request.images"), "media", ref("media.value"))),
      seed: omit(ref("request.providerOptions.volcengine-jimeng-image.seed")),
      width: omit(ref("request.output.width")),
      height: omit(ref("request.output.height")),
      req_json: omit(
    ...
    files: [
      {
        ...
        source: first(
          filter(
            sorted(ref("request.images")),
            "media",
            ne(ref("media.role"), "mask"),
          ),
        ),
      ...
      media: omit({
        $concatArrays: [
          map(sorted(ref("request.images")), "media", {
            type: coalesce(ref("media.role"), "reference_image"),
            url: ref("media.value"),
          }),
          map(sorted(ref("request.videos")), "media", {
            type: coalesce(ref("media.role"), "reference_video"),
            url: ref("media.value"),
          }),
          map(sorted(ref("request.audios")), "media", {
            type: coalesce(ref("media.role"), "reference_voice"),
            url: ref("media.value"),
          }),
        ],
      }),
...
add({
  ...
  create: jsonCreate("/v1/videos", {
    ...
    prompt: ref("request.prompt"),
    seconds: { $toString: ref("request.duration") },
    size: omit({ $upper: ref("request.resolution") }),
    aspect_ratio: omit(ref("request.aspectRatio")),
    reference_images: omit(
      map(sorted(ref("request.images")), "media", {
        url: ref("media.value"),
        role: coalesce(ref("media.role"), "reference_image"),
      }),
    ),
    reference_videos: omit(
      map(sorted(ref("request.videos")), "media", {
        url: ref("media.value"),
        duration: omit({
          $multiply: [ref("media.metadata.durationMs"), 0.001],
        }),
      }),
    ),
    reference_audios: omit(
      map(sorted(ref("request.audios")), "media", { url: ref("media.value") }),
    ),
  }),
  poll: { method: "GET", path: "/v1/videos/{{taskId}}" },
  result: {
...
add({
  ...
  validations: [
    {
      assert: {
        $lte: [
          len(
            filter(ref("request.images"), "media", {
              $in: [ref("media.role"), ["first_frame", "last_frame"]],
            }),
          ),
          1,
        ],
      },
      message: "xAI Video 最多支持一个帧输入，当前 profile 不支持尾帧",
    },
    {
      assert: {
        $not: {
            ...
            gt(
              len(
                filter(ref("request.images"), "media", {
                  $in: [
                    ref("media.role"),
                    ["reference_image", "subject_reference", "style_reference"],
                  ],
                }),
              ),
              0,
      ...
      message: "xAI Video 不能同时混用起始帧和角色参考图",
    },
  ],
  create: jsonCreate("/v1/videos/generations", {
    ...
    image: conditional(
      gt(len(mediaWithRoles("request.images", ["first_frame", ""])), 0),
      {
        url: firstMediaFieldWithRoles(
          "request.images",
          ["first_frame", ""],
...
add({
  ...
  validations: [
    {
        ...
        $eq: [
          len(
            filter(
              ref("request.images"),
              "media",
              eq(ref("media.role"), "reference_image"),
            ),
          ),
          0,
        ],
      ...
      message:
        "Gemini Veo 当前 profile 不支持角色参考图，请使用首帧输入或支持 reference_to_video 的协议",
    },
  ],
  create: jsonCreate("/v1beta/models/{{model}}:predictLongRunning", {
        ...
        image: conditional(
          ...
          conditional(
            ...
            ),
            {
              inlineData: {
                mimeType: {
                  $dataMime: firstMediaFieldWithRoles(
                    "request.images",
                    ["first_frame", ""],
                    "dataUrl",
                  ),
                ...
                data: {
                  $dataPayload: firstMediaFieldWithRoles(
                    "request.images",
                    ["first_frame", ""],
                    "dataUrl",
                  ),
            ...
            {
              fileUri: firstMediaFieldWithRoles(
                "request.images",
                ["first_frame", ""],
                "url",
              ),
    ...
    parameters: {
      aspectRatio: omit(ref("request.aspectRatio")),
      durationSeconds: omit(ref("request.duration")),
      resolution: omit(ref("request.resolution")),
      generateAudio: ref("request.generateAudio"),
      sampleCount: omit(ref("request.imageCount")),
      negativePrompt: omit(
        ref("request.providerOptions.gemini-veo.negativePrompt"),
      ),
      personGeneration: omit(
        ref("request.providerOptions.gemini-veo.personGeneration"),
      ),
      seed: omit(ref("request.providerOptions.gemini-veo.seed")),
    },
  }),
  poll: { method: "GET", path: "/v1beta/{{taskId}}" },
  response: asyncResponse("video", {
    taskId: coalesce(ref("response.name"), ref("taskId")),
    status: conditional(
      ref("response.error"),
      "failed",
      conditional(ref("response.done"), "succeeded", "processing"),
    ),
    videos: coalesce(
    ...
    message: ref("response.error.message"),
  }),
});

add({
  id: "novita-video",
  providerId: "novita-video",
  name: "Novita Video",
  vendor: "Novita",
  capability: "video",
  ...
  create: jsonCreate("/v3/video/create", {
    ...
    aspect_ratio: coalesce(ref("request.aspectRatio"), "16:9"),
    image_url: omit(
      firstMediaFieldWithRoles("request.images", ["first_frame", ""], "value"),
    ),
    seed: omit(ref("request.providerOptions.novita-video.seed")),
...
add({
  ...
  providerId: "volcengine-jimeng-video",
  ...
  create: jsonCreate(
    "/",
    {
      ...
      image_urls: omit(
        map(
          { $sortByOrder: ref("request.images") },
          "media",
          ref("media.value"),
        ),
      ),
  ...
  poll: {
    ...
    contentType: "application/json",
    query: { Action: "CVSync2AsyncGetResult", Version: "2022-08-31" },
    body: { req_key: ref("request.model"), task_id: ref("taskId") },
  },
  response: asyncResponse("video", {
    taskId: coalesce(
      ref("response.data.task_id"),
...
const agnesFrameImages = mediaWithRoles("request.images", [
  "first_frame",
  "last_frame",
]);
const agnesReferenceImages = mediaWithRoles("request.images", [
  "reference_image",
  "subject_reference",
  "style_reference",
]);
const agnesReferenceMode = {
  $or: [
    gt(len(agnesReferenceImages), 0),
...
add({
  ...
  validations: [
    ...
    },
    {
      assert: {
        $and: [
          { $gte: [ref("request.duration"), 4] },
          { $lte: [ref("request.duration"), 12] },
        ],
      },
    ...
    },
    {
      assert: {
        $in: [
          { $upper: coalesce(ref("request.resolution"), "720P") },
          ["720P", "960P", "2K"],
        ],
      },
    ...
    {
      assert: {
        $not: { $and: [gt(len(agnesFrameImages), 0), agnesReferenceMode] },
      },
    ...
    },
    {
      assert: {
        $or: [
          ne(ref("request.model"), "agnes-video-2.5-flash"),
          eq({ $upper: coalesce(ref("request.resolution"), "720P") }, "720P"),
        ],
      },
  ...
  create: jsonCreate("/videos", {
    ...
    mode: conditional(
      agnesReferenceMode,
      "reference",
      conditional(gt(len(ref("request.images")), 0), "keyframe", "text"),
    ),
    seconds: { $toString: ref("request.duration") },
    size: { $upper: coalesce(ref("request.resolution"), "720P") },
    aspect_ratio: coalesce(ref("request.aspectRatio"), "16:9"),
    n: coalesce(ref("request.output.count"), 1),
    first_frame: conditional(
      { $not: agnesReferenceMode },
      omit(
        coalesce(
          firstMediaFieldWithRoles("request.images", ["first_frame"], "value"),
          first(
            map(sorted(ref("request.images")), "media", ref("media.value")),
          ),
        ),
      ),
      null,
    ),
    last_frame: conditional(
      { $not: agnesReferenceMode },
      omit(firstMediaFieldWithRoles("request.images", ["last_frame"], "value")),
      null,
    ),
    images: conditional(
      agnesReferenceMode,
      omit(map(sorted(ref("request.images")), "media", ref("media.value"))),
      null,
    ),
    videos: conditional(
      agnesReferenceMode,
    ...
    audios: conditional(
      agnesReferenceMode,
      omit(map(sorted(ref("request.audios")), "media", ref("media.value"))),
      null,
    ),
  }),
  poll: {
    ...
    path: "/agnesapi",
    originPath: true,
    query: { video_id: ref("taskId"), model_name: ref("request.model") },
  },
  response: asyncResponse("video", {
    taskId: coalesce(
...
add({
  ...
  validations: [
    {
      assert: eq(ref("request.model"), "agnes-video-v2.0"),
      message: "Agnes Video V2.0 插件仅支持 agnes-video-v2.0",
    },
    {
      assert: eq(len(ref("request.videos")), 0),
      message: "Agnes Video V2.0 不支持参考视频",
    },
    {
      assert: eq(len(ref("request.audios")), 0),
      message: "Agnes Video V2.0 不支持参考音频",
    },
    {
      assert: eq(len(agnesReferenceImages), 0),
      message: "Agnes Video V2.0 不支持角色或风格参考图",
    },
    {
      assert: { $lte: [len(agnesFrameImages), 2] },
  ...
  create: jsonCreate("/videos", {
      ...
      $min: [
        441,
        {
          $add: [
            {
              $ceilStep: [
                { $multiply: [coalesce(ref("request.duration"), 5), 24] },
                8,
              ],
            },
            1,
    ...
    extra_body: conditional(gt(len(agnesFrameImages), 1), {
      image: map(agnesFrameImages, "media", ref("media.value")),
      mode: "keyframes",
    }),
  }),
  poll: {
...
add({
  ...
  create: {
    method: "POST",
    pathTemplate: coalesce(
      ref("request.providerOptions.stability-image.endpoint"),
    ...
    body: {
      prompt: ref("request.prompt"),
      negative_prompt: omit(
        ref("request.providerOptions.stability-image.negative_prompt"),
      ),
      aspect_ratio: omit(ref("request.aspectRatio")),
      output_format: coalesce(
        ref("request.output.format"),
        ref("request.providerOptions.stability-image.output_format"),
        "png",
      ),
      seed: omit(ref("request.providerOptions.stability-image.seed")),
      style_preset: omit(
        ref("request.providerOptions.stability-image.style_preset"),
      ),
      strength: omit(ref("request.providerOptions.stability-image.strength")),
      cfg_scale: omit(ref("request.providerOptions.stability-image.cfg_scale")),
      grow_mask: omit(ref("request.providerOptions.stability-image.grow_mask")),
      creativity: omit(
        ref("request.providerOptions.stability-image.creativity"),
      ),
    },
    files: [
      {
      ...
      {
        name: "mask",
        source: first(mediaWithRoles("request.images", ["mask"])),
        filename: "mask.png",
      },
    ],
  },
    ...
    images: conditional(ref("response.image"), [
      {
        dataUrl: {
          $concat: [
            "data:",
            coalesce(ref("response.mime_type"), "image/png"),
            ";base64,",
            ref("response.image"),
          ],
        },
...
add({
  ...
  create: {
    ...
    body: {
      ...
      aspect_ratio: omit(ref("request.aspectRatio")),
      resolution: omit(ref("request.resolution")),
      num_images: coalesce(ref("request.imageCount"), 1),
      rendering_speed: omit(
      ...
      seed: omit(ref("request.providerOptions.ideogram-image.seed")),
      color_palette: omit(
        ref("request.providerOptions.ideogram-image.color_palette"),
      ),
      character_reference_images_mask: omit(
        ref(
          "request.providerOptions.ideogram-image.character_reference_images_mask",
        ),
      ),
    },
    files: [
  ...
  response: {
    status: "succeeded",
    images: coalesce(ref("response.data"), ref("response.images")),
    errorPaths: ["error.code"],
...
add({
  ...
  providerId: "dashscope-wan-video",
  ...
  validations: [
    {
      assert: {
        $lte: [len(mediaWithRoles("request.images", ["first_frame"])), 1],
      },
      message: "Wan 视频最多只能有一个首帧",
    },
    {
      assert: {
        $lte: [len(mediaWithRoles("request.images", ["last_frame"])), 1],
      },
    ...
    },
    {
      assert: {
        $or: [
          eq(len(mediaWithRoles("request.images", ["last_frame"])), 0),
          eq(len(mediaWithRoles("request.images", ["first_frame"])), 1),
        ],
      },
  ...
  create: jsonCreate(
    "/api/v1/services/aigc/video-generation/video-synthesis",
    {
      ...
      input: {
        ...
        img_url: omit(
          conditional(
            ...
            coalesce(
              ...
              firstMediaFieldWithRoles(
                "request.images",
                ["reference_image", "edit_source", ""],
                "value",
              ),
            ),
          ),
        ),
        first_frame_url: omit(
          conditional(
            gt(len(mediaWithRoles("request.images", ["last_frame"])), 0),
            firstMediaFieldWithRoles(
              "request.images",
              ["first_frame"],
              "value",
        ...
        last_frame_url: omit(
          firstMediaFieldWithRoles("request.images", ["last_frame"], "value"),
        ),
        reference_images: omit(
          conditional(
            eq(ref("request.operation"), "reference_to_video"),
            map(
              mediaWithRoles("request.images", [
                "reference_image",
                "subject_reference",
      ...
      parameters: {
        size: omit(ref("request.resolution")),
        duration: omit(ref("request.duration")),
        prompt_extend: omit(
          ref("request.providerOptions.dashscope-wan-video.prompt_extend"),
        ),
        watermark: ref("request.watermark"),
        seed: omit(ref("request.providerOptions.dashscope-wan-video.seed")),
        shot_type: omit(
          ref("request.providerOptions.dashscope-wan-video.shot_type"),
        ),
        audio: ref("request.generateAudio"),
        template: omit(
  ...
  response: asyncResponse("video", {
    ...
    usage: ref("response.usage"),
    errorPaths: ["code"],
    messagePaths: ["message", "output.message"],
  }),
});
...
add({
  ...
  create: jsonCreate(
    "/api/v1/services/aigc/text2image/image-synthesis",
    {
      ...
      parameters: {
        size: omit(ref("request.aspectRatio")),
        n: coalesce(ref("request.imageCount"), 1),
        seed: omit(ref("request.providerOptions.dashscope-wanx-image.seed")),
        style: omit(ref("request.providerOptions.dashscope-wanx-image.style")),
        prompt_extend: omit(
...
for (const [
  ...
  [
    ...
    "/v1/projects/{{request.providerOptions.vertex-gemini.project}}/locations/{{request.providerOptions.vertex-gemini.location}}/publishers/google/models/{{model}}:generateContent",
    null,
  ...
  [
    ...
    "image",
    "https://{location}-aiplatform.googleapis.com",
    "/v1/projects/{{request.providerOptions.vertex-imagen.project}}/locations/{{request.providerOptions.vertex-imagen.location}}/publishers/google/models/{{model}}:predict",
    null,
  ...
  [
    ...
    "video",
    "https://{location}-aiplatform.googleapis.com",
    "/v1/projects/{{request.providerOptions.vertex-veo.project}}/locations/{{request.providerOptions.vertex-veo.location}}/publishers/google/models/{{model}}:predictLongRunning",
    "/v1/{{taskId}}",
    "Vertex long-running operation",
  ],
  [
    "seedance-videos-compatible",
    "Seedance Compatible /videos",
    "Gateway profile",
    "video",
    "http://127.0.0.1:3000",
    "/v1/videos",
    "/v1/videos/{{taskId}}",
    "gateway-specific videos JSON",
  ],
  [
    "kling-video",
    "Kling Video",
  ...
  [
    ...
    "/v1/image_to_video",
    "/v1/tasks/{{taskId}}",
    "promptImage+promptText+ratio+duration",
  ],
  [
    "luma-dream-machine",
    "Luma Dream Machine",
  ...
  [
    ...
    "/v1/videos",
    "/v1/tasks/{{taskId}}",
    "text/image/reference video task",
  ],
  [
    "pixverse-video",
    "PixVerse Video",
  ...
  [
    ...
    "/v4/async-result/{{taskId}}",
    "model+prompt+image_url+quality+with_audio",
  ],
  [
    "baidu-video",
    "百度千帆视频",
  ...
  const standardFields =
    capability === "text"
      ? {
          model: ref("request.model"),
          messages: ref("request.messages"),
          input: omit(ref("request.prompt")),
          parameters: omit(ref(`${optionPath}.parameters`)),
        }
      : capability === "image"
        ? {
            model: ref("request.model"),
            prompt: ref("request.prompt"),
            images: omit(
              map(ref("request.images"), "media", ref("media.value")),
            ),
            parameters: omit(ref(`${optionPath}.parameters`)),
          }
        : {
            ...
            videos: omit(
              map(ref("request.videos"), "media", ref("media.value")),
            ),
            audios: omit(
              map(ref("request.audios"), "media", ref("media.value")),
            ),
            duration: omit(ref("request.duration")),
            aspect_ratio: omit(ref("request.aspectRatio")),
            resolution: omit(ref("request.resolution")),
  ...
  const standard = {
    $merge: [
      standardFields,
      coalesce(ref(`${optionPath}.body`), ref(`${optionPath}.extra_body`), {}),
    ],
  };
  let auth = bearer;
  let configuration;
  let operationExtra = id.startsWith("dashscope-")
    ? { headers: { "X-DashScope-Async": "enable" } }
    : {};
  if (id.startsWith("aws-bedrock-")) {
    ...
    configuration = {
      fields: [
        { name: "apiKey", type: "secret", label: "SecretId", required: true },
        {
          name: "secretKey",
          type: "secret",
          label: "SecretKey",
    ...
    operationExtra = {
      headers: {
        "X-TC-Action": ref(`${optionPath}.action`),
        "X-TC-Version": ref(`${optionPath}.version`),
      },
    };
    ...
    operationExtra = {
      query: {
        "api-version": coalesce(ref(`${optionPath}.api-version`), "2024-10-21"),
      },
    };
  }
  add({
    ...
    response:
      capability === "text"
        ...
        : pollPath
          ? asyncResponse(capability)
          : {
              status: "succeeded",
              [capability + "s"]: coalesce(
                ref("response.data"),
                ref("response.output"),
                ref("response.url"),
              ),
...
for (const [id, name, capability, createPath, pollPath, resultPath] of [
  [
    ...
    "image",
    "/{{model}}",
    "/{{request.providerOptions.fal-queue-image.statusPath}}",
    "response.images",
  ],
  [
    "fal-queue-video",
    ...
    "video",
    "/{{model}}",
    "/{{request.providerOptions.fal-queue-video.statusPath}}",
    "response.video",
  ],
  [
  ...
  [
    ...
    "video",
    "/{{model}}",
    "/{{request.providerOptions.pika-via-fal.statusPath}}",
    "response.video",
  ],
]) {
  const optionPath = `request.providerOptions.${id}`;
  add({
    ...
    providerId: id,
    name,
    vendor: id.includes("replicate")
      ? "Replicate"
      : id.includes("comfy")
        ? "ComfyUI"
        : id.includes("runninghub")
          ? "RunningHub"
          : "fal.ai",
    capability,
    baseUrl: id.includes("replicate")
      ? "https://api.replicate.com"
      : id.includes("fal") || id.includes("pika")
        ? "https://queue.fal.run"
        : "http://127.0.0.1:8188",
    ...
    create: jsonCreate(createPath, {
      version: omit(ref(`${optionPath}.version`)),
      model: omit(coalesce(ref(`${optionPath}.model`), ref("request.model"))),
      input: omit(
        coalesce(ref(`${optionPath}.input`), {
          prompt: ref("request.prompt"),
          images: map(ref("request.images"), "media", ref("media.value")),
          videos: map(ref("request.videos"), "media", ref("media.value")),
          audios: map(ref("request.audios"), "media", ref("media.value")),
        }),
      ),
      prompt: omit(ref(`${optionPath}.workflow`)),
      client_id: omit(ref(`${optionPath}.client_id`)),
      webhook: omit(ref(`${optionPath}.webhook`)),
      webhook_events_filter: omit(ref(`${optionPath}.webhook_events_filter`)),
    }),
    poll: pollPath ? { method: "GET", path: pollPath } : undefined,
    cancel: id.includes("replicate")
      ? { method: "POST", path: "/v1/predictions/{{taskId}}/cancel" }
      : undefined,
    response: asyncResponse(capability, {
      [capability + "s"]: ref(resultPath),
      taskId: coalesce(
        ref("response.id"),
...
function collectTemplateFields(value, prefix, rows) {
  ...
  if (Array.isArray(value)) {
    ...
    value.forEach((item, index) =>
      collectTemplateFields(item, `${prefix}[${index}]`, rows),
    );
    return;
  }
...
function operationFieldRows(operation, label) {
  ...
  const rows = [
    ...
    [
      `${label}.contentType`,
      JSON.stringify(operation.contentType || "application/json"),
    ],
  ];
...
function docsFor(spec) {
  const rows = spec.params
    .map(
      ([name, type, required, mapping, description]) =>
        `| \`${name}\` | ${type} | ${required ? "是" : "否"} | \`${mapping}\` | ${description} |`,
    )
    .join("\n");
  const operationRows = [
    ...
    .map(
      ([path, expression]) =>
        `| \`${path}\` | \`${String(expression).replaceAll("|", "\\|")}\` |`,
    )
    .join("\n");
  const responseRows = [];
  ...
  const mappedResponseRows = responseRows
    .map(
      ([path, expression]) =>
        `| \`${path}\` | \`${String(expression).replaceAll("|", "\\|")}\` |`,
    )
  ...
  const manifestJSON = JSON.stringify({
    ...
    result: spec.result,
    response: spec.response,
    agentResponse: spec.agentResponse,
  });
  const optionRefs = [
    ...new Set(
      [
        ...manifestJSON.matchAll(
          new RegExp(
            `request\\.providerOptions\\.${spec.providerId.replaceAll("-", "\\-")}\\.([A-Za-z0-9_.-]+)`,
            "g",
          ),
        ),
      ].map((match) => match[1]),
  ...
  const optionRows = optionRefs.length
    ? optionRefs
        .map((name) => `- \`providerOptions.${spec.providerId}.${name}\``)
        .join("\n")
    : "- 无额外扩展键。";
  const configRows = (spec.configuration || config()).fields
    .map(
      (field) =>
        `| \`${field.name}\` | ${field.type} | ${field.required ? "是" : "否"} | ${field.label || ""} |`,
    )
    .join("\n");
...
const selectedSpecs =
  requestedPackageIDs.size > 0
    ? specs.filter((spec) => requestedPackageIDs.has(spec.id))
    : specs;
if (
  requestedPackageIDs.size > 0 &&
  selectedSpecs.length !== requestedPackageIDs.size
) {
